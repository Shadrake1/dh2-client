...
class RandomTeams {
  ...
  getAbility(types, moves, abilities, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    if (species.id === "cinccino")
      return role === "Wallbreaker" ? "Skill Link" : "Technician";
  ...
  getPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
    if (!isDoubles) {
      if (!isLead && role === "Bulky Setup" && (ability === "As One (OU)" || ability === "Iron Thorn")) {
        return "Booster Energy";
      }
    }
    if (role === "Mega") {
      if (species.id === "necromaneduskmane")
        return "Depleted Ultranecrozmium Z";
      if (species.id === "woopquaza")
    ...
    if (role === "AV Pivot")
      return "Assault Vest";
    if (species.id === "hooporantunbound")
      return "Light Ball";
    if (species.id === "zacianoh")
      return "Rusted Sword";
    if (species.id === "shitto")
      return "Black Sludge";
    if (species.id === "swalos")
      return "Starf Berry";
    if (species.id === "koraisle" || species.id === "mirainun")
      return "Destiny Knot";
    if (species.id === "tipplin")
      return "Metronome";
    if (species.id === "mimighold")
      return "Lum Berry";
    if (species.id === "zamadactylcrowned")
      return "Rusted Shield";
    if (species.id === "basbal" || species.id === "naclinch" || species.id === "glimmgar" || species.id === "roseron")
      return "Eviolite";
    if (species.id === "exeggumoramora")
      return "White Herb";
    if (species.id === "rolyklawfy")
      return "Focus Sash";
    if (species.id === "necromaneduskmane" || species.id === "ferrothorns")
      return "Booster Energy";
    if (species.id === "gigawrath")
      return "Choice Band";
    if (species.id === "yvenne")
      return this.sample(["Shell Bell", "Life Orb", "Leftovers", "Heavy-Duty Boots"]);
    if (species.id === "raichudus" && moves.has("swagger"))
      return "lumberry";
    if (species.id === "chiruno" && moves.has("sheercold"))
      return "choicespecs";
    if (species.id === "blazer")
      return "Focus Sash";
    if (species.id === "spinningfire")
      return "Berserk Gene";
    if (species.id === "snorcannon")
      return "Starf Berry";
    if (species.id === "tapuluna")
      return "Life Orb";
    if (species.id === "smearccino")
      return "Focus Sash";
    ...
    if (moves.has("scaleshot") || species.id === "torterra" && !isDoubles || species.id === "cinccino" && role !== "Wallbreaker")
      return "Loaded Dice";
  ...
  getDoublesItem(ability, types, moves, counter, teamDetails, species, isLead, teraType, role) {
    ...
    if ((offensiveRole || role === "Tera Blast user" && (species.baseStats.spe >= 80 || moves.has("trickroom"))) && (!moves.has("fakeout") || species.id === "ambipom") && !moves.has("incinerate") && (!moves.has("uturn") || types.includes("Bug") || species.baseStats.atk >= 120 || ability === "Libero") && (!moves.has("icywind") && !moves.has("electroweb") || species.id === "ironbundle")) {
      return (ability === "As One (OU)" || ability === "Ancient Pyramid") && ["firstimpression", "uturn", "voltswitch"].every((m) => !moves.has(m)) && species.id !== "ironvaliant" ? "Booster Energy" : "Life Orb";
    }
  ...
  getItem(ability, types, moves, counter, teamDetails, species, isLead, teraType, role) {
    if (types.includes("Normal") && moves.has("fakeout"))
      return "Silk Scarf";
    if (species.id !== "jirachi" && counter.get("Physical") >= 4 && ["fakeout", "firstimpression", "flamecharge", "rapidspin", "ruination", "superfang"].every((m) => !moves.has(m))) {
    ...
    if (counter.get("speedsetup") && role === "Bulky Setup")
      return "Weakness Policy";
    if (!counter.get("Status") && (moves.has("rapidspin") || !["Fast Attacker", "Wallbreaker", "Tera Blast user"].includes(role))) {
      return "Assault Vest";
    }
    ...
    if (role === "Fast Support" && isLead && !counter.get("recovery") && !counter.get("recoil") && species.baseStats.hp + species.baseStats.def + species.baseStats.spd < 258)
      return "Focus Sash";
    if (!["Fast Attacker", "Wallbreaker", "Tera Blast user"].includes(role) && ability !== "Levitate" && this.dex.getEffectiveness("Ground", species) >= 2)
      return "Air Balloon";
  ...
  getPokemonPool(type, pokemonToExclude = [], isMonotype = false, pokemonList) {
    ...
    for (const pokemon of pokemonList) {
      ...
      if (exclude.includes(species.id) && pokemonList.length != 1)
        continue;
      if (isMonotype) {
    ...
    if (pokemonPool.length == 0)
      pokemonPool.push(pokemonList[0]);
  ...
  randomTeam() {
    ...
    while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
      ...
      if (species.baseSpecies === "Koraisle" && !teamDetails.mirainun)
        teamDetails.koraisle = 1;
      if (species.baseSpecies === "Mirainun" && !teamDetails.koraisle)
        teamDetails.mirainun = 1;
      if (species.baseSpecies === "Beetar" && !teamDetails.scolislash)
        teamDetails.beetar = 1;
      if (species.baseSpecies === "Scolislash" && !teamDetails.beetar)
        teamDetails.scolislash = 1;
      if (species.baseSpecies === "Darkcross" && !teamDetails.darkcross)
        teamDetails.darkcross = 1;
      if (species.baseSpecies === "Chiruno" && !teamDetails.avalluxe)
        teamDetails.beetar = 1;
      if (species.baseSpecies === "Avalluxe" && !teamDetails.chiruno)
        teamDetails.avalluxe = 1;
      if (species.baseSpecies === "Kingcario")
        teamDetails.dark = true;
      const types = species.types;
