...
const Abilities = {
    ...
    onSwitchIn(pokemon) {
      for (const target of pokemon.foes()) {
        for (const moveSlot of target.moveSlots) {
          ...
          if (!pokemon.hasAbility("anticipation"))
            return;
  ...
  terashift: {
    ...
    onPreStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Terapagos")
        return;
      if (pokemon.getItem() === "Special Tera Orb")
        return;
  ...
  merciless: {
    ...
    onModifyCritRatio(critRatio, source, target) {
      if (target && ["psn", "tox", "brn", "par"].includes(target.status))
        return 5;
    },
  ...
  soulheart: {
    ...
    onAnyFaint() {
      if (this.effectState.target.swordBoost)
        return;
  ...
  middleeight: {
    ...
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.baseSpecies !== "Meloetta" || attacker.transformed)
        return;
      if (move.category === "Status")
        return;
  ...
  stancechange: {
    ...
    onModifyMove(move, attacker, defender) {
      ...
      if (targetForme === "Aegislash-Blade")
        move.basePower = move.basePower * 1.2;
  ...
  aerodynamism: {
    ...
    onSourceModifyAccuracy(accuracy, target, source, move) {
      if (move.flags["wind"] && typeof accuracy === "number")
        return true;
      return accuracy;
    },
  ...
  karate: {
    ...
    onModifyType(move, pokemon) {
      ...
      if (move.type === "Normal" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized) && !(move.name === "Tera Blast" && pokemon.hasItem("legendplate"))) {
        move.type = "Fighting";
    ...
    onResidual(pokemon) {
      ...
      for (const target of this.getAllActive()) {
        if (pokemon.volatiles["lifestealer"]) {
          const damage = this.damage(this.clampIntRange(pokemon.baseMaxhp / 16, 1) * this.effectState.stage, pokemon, target);
          if (damage) {
  ...
  savage: {
    shortDesc: "The Pok\xE9mon\u2019s Attack or Special Attack copies from the higher stat (held items does not apply for which is higher). Stat stages and held items apply as normal.",
    onModifyAtkPriority: 5,
    onModifyAtk(atk, pokemon) {
  ...
  pickup: {
    ...
    onResidual(pokemon) {
      if (pokemon.item)
        return;
      const pickupTargets = this.getAllActive().filter((target) => target.lastItem && target.usedItemThisTurn && pokemon === target);
  ...
  resourceful: {
    ...
    name: "Resourceful",
    rating: 4,
    desc: "If this Pok\xE9mon's item would trigger, it triggers again. Once per battle, if this Pok\xE9mon's item would be consumed, it isn't.",
