...
const Items = {
  ...
  boosterenergy: {
    inherit: true,
    onUpdate(pokemon) {
      if (!this.effectState.started || pokemon.transformed || this.queue.peek(true)?.choice === "runSwitch")
        return;
  ...
  garchompite: {
    ...
    itemUser: ["Garpyuku"],
    onTakeItem(item, source) {
      if ([item.megaEvolves, "Chomptry"].includes(source.baseSpecies.baseSpecies))
        return false;
      return true;
    },
    num: 683,
  ...
  scizorite: {
    ...
    itemUser: ["Druddizor"],
    onTakeItem(item, source) {
      if ([item.megaEvolves, "Tentazor"].includes(source.baseSpecies.baseSpecies))
        return false;
      return true;
    },
    num: 670,
  ...
  quickclaw: {
    ...
    onFractionalPriority(priority, pokemon, target, move) {
      if (move.category === "Status" && pokemon.hasAbility(["myceliummight", "galvanicrelay"]))
        return;
  ...
  aerodactylite: {
    ...
    itemUser: ["Aero Wake"],
    onTakeItem(item, source) {
      if ([item.megaEvolves, "Aerodirge"].includes(source.baseSpecies.baseSpecies))
        return false;
      return true;
    },
    num: 672,
  ...
  hearthflamemask: {
    ...
    onTakeItem(item, source) {
      if (source.baseSpecies.baseSpecies === "Hattepon")
        return false;
      return true;
    },
