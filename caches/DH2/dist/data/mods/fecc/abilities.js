...
const randomSets = require("../../random-battles/gen9/sets.json");
const usergroups = {};
...
function deepCopyInto(obj1, obj2) {
  ...
  */
  ...
  asonesex: {
    onPreStart(pokemon) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName("anaconja")}|What the hell? Parasex?`);
    },
  ...
  loosefuse: {
    onSourceDamagingHit(damage, target, source, move) {
      ...
      if (this.randomChance(1, 10)) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(source.name)}|Muck this world.`);
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(source.name)}|That is my final message`);
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(source.name)}|Good bye.`);
    ...
    onStart(pokemon) {
      ...
      for (const target of pokemon.side.foe.active) {
        if (!target || target.fainted)
          continue;
        if (!activated)
          this.add("-ability", pokemon, "Do The Roar");
        activated = true;
        let statName = "atk";
        let bestStat = 0;
        let s;
  ...
  riptides: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Zyggizaggi" || pokemon.level < 20 || pokemon.transformed || !pokemon.hp)
        return;
      ...
      if (pokemon.species.baseStats.hp > previousFormeHP)
        changeHP = true;
      if (changeHP) {
        pokemon.baseMaxhp = Math.floor(Math.floor(
          2 * pokemon.species.baseStats["hp"] + pokemon.set.ivs["hp"] + Math.floor(pokemon.set.evs["hp"] / 4) + 100
  ...
  pillage: {
    ...
    onUpdate(pokemon) {
      ...
      if (!this.runEvent("SetAbility", target, pokemon, this.effect, pillageAbil) || !this.runEvent("SetAbility", pokemon, pokemon, this.effect, ability))
        return;
  ...
  weatherwar: {
    ...
    onResidual(pokemon) {
      ...
      if (pokemon.species.baseSpecies !== "Morform" || pokemon.terastallized)
        return;
        ...
        move.secondaries.push(
          ...
          //eerie spell
          {
            ...
            onHit(target) {
              if (!target.hp)
                return;
              let move2 = target.lastMove;
              if (!move2 || move2.isZ)
                return;
              if (move2.isMax && move2.baseMove)
                move2 = this.dex.moves.get(move2.baseMove);
              const ppDeducted = target.deductPP(move2.id, 3);
              if (!ppDeducted)
                return;
              this.add("-activate", target, "move: Eerie Spell", move2.name, ppDeducted);
            }
          },
          //flinch
          {
          ...
          {
            chance: 10,
            onAfterHit(target, source, move2) {
              if (!move2.hasSheerForce && source.hp) {
                for (const side of source.side.foeSidesWithConditions()) {
                  side.addSideCondition("stealthrock");
                }
              }
            },
            onAfterSubDamage(damage, target, source, move2) {
              if (!move2.hasSheerForce && source.hp) {
                for (const side of source.side.foeSidesWithConditions()) {
                  side.addSideCondition("stealthrock");
                }
          ...
          {
            chance: 10,
            onHit(target, source, move2) {
              if (source.isActive)
                target.addVolatile("trapped", source, move2, "trapper");
            }
          }
        );
      }
  ...
  shockandballtorture: {
    ...
    onDamagingHit(damage, target, source, move) {
      ...
      for (const pokemon of [target, source]) {
        if (pokemon.volatiles["curseofshocking"])
          continue;
        if (!announced) {
  ...
  quickstart: {
    ...
    condition: {
      duration: 5,
      onStart(target) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${target.name}|Faster than fast, Quicker than quick. I am lightning.`);
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${target.name}|Speed. I am speed.`);
  ...
  beewitch: {
    onAfterMoveSecondarySelf(source, target, move) {
      if (!move || !target || !target.hp || source.switchFlag === true)
        return;
      if (target !== source && move.category !== "Status") {
  ...
  metamorphic: {
    onStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Miniluna" || pokemon.transformed)
        return;
      if (!pokemon.baseSpecies.forme)
        pokemon.formeChange("Miniluna-Bloodstone");
    },
    ...
    onAfterMove(target, source, move) {
      if (target.species.baseSpecies !== "Miniluna" || target.transformed)
        return;
      if (move.category === "Status" || !["Normal", "Fighting"].includes(move.type))
        return;
  ...
  teratooth: {
    onPreStart(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Teradoof")
        return;
  ...
  terashell: {
    ...
    onEffectiveness(typeMod, target, type, move) {
      if (!target || target.species.name !== "Teradoof-Terastal")
        return;
  ...
  teraformzero: {
    ...
    onAfterTerastallization(pokemon) {
      if (pokemon.baseSpecies.name !== "Teradoof-Stellar")
        return;
  ...
  gorillaofruin: {
    ...
    onAnyModifySpA(spa, source, target, move) {
      ...
      if (source.hasAbility("Gorilla of Ruin"))
        return;
      if (!move.ruinedSpA)
    ...
    onFoeSwitchIn(pokemon) {
      if (!pokemon.hasAbility("Gorilla of Ruin") && !pokemon.volatiles["gorillaofruin"])
        pokemon.addVolatile("gorillaofruin");
    },
    condition: {
      onStart(pokemon) {
  ...
  swallowswallow: {
    onModifyDamage(damage, source, target, move) {
      if (move && target.getMoveHitData(move).typeMod > 0) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${source.name}|I'm Smart`);
      } else if (target.getMoveHitData(move).typeMod < 0) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${source.name}|I'm Dumb`);
      }
    },
    onTryHit(target, source, move) {
      if (move.type === "Flying" && target !== source) {
        this.add("-immune", target, "[from] ability: Swallow Swallow");
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${target.name}|mmmm... tastes like chicken`);
        return null;
      }
    },
    flags: { breakable: 1 },
    ...
    onStart(pokemon) {
      ...
      for (const target of pokemon.side.foe.active) {
        if (!target || target.fainted)
          continue;
        if (!activated)
          this.add("-ability", pokemon, "Ultra Yoink");
        activated = true;
        let statName = "atk";
        let bestStat = 0;
        let s;
        ...
        if (target.volatiles["substitute"]) {
          ...
          if (statName === "atk") {
            this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(pokemon.name)}|Your Attack? No, OUR Attack`);
            pokemon.storedStats.atk = target.storedStats.atk;
          }
          if (statName === "def") {
            this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(pokemon.name)}|Your Defense? No, OUR Defense`);
            pokemon.storedStats.def = target.storedStats.def;
          }
          if (statName === "spa") {
            this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(pokemon.name)}|Your Special Attack? No, OUR Special Attack`);
            pokemon.storedStats.spa = target.baseSpecies.baseStats.spa;
          }
          if (statName === "spd") {
            this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(pokemon.name)}|Your Special Defense? No, OUR Special Defense`);
            pokemon.storedStats.spd = target.storedStats.spd;
          }
          if (statName === "spe") {
            this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(pokemon.name)}|Your Speed? No, OUR Speed`);
  ...
  graceofruin: {
    onStart(pokemon) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName("Earlio2")}|wtf`);
    },
    onModifySecondaries(secondaries) {
  ...
  ironthorn: {
    ...
    condition: {
      ...
      onResidual(pokemon) {
        for (const target of pokemon.adjacentFoes()) {
          this.add(`c:|${Math.floor(Date.now() / 1e3)}|${pokemon.name}|I know what you are...`);
  ...
  bidenblast: {
    ...
    onFaint(pokemon) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${pokemon.name}|It's So Joever`);
    },
    flags: {},
    name: "Biden Blast"
  ...
  "tier1boomer": {
    onStart(pokemon) {
      ...
      for (const target of pokemon.adjacentFoes()) {
        const explosion = this.dex.getActiveMove("explosion");
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${pokemon.name}|I just wanna boom for Arceus' sake!`);
  ...
  atv: {
    onStart(pokemon) {
      ...
      if (!pokemon.setType(newBaseTypes))
        return;
  ...
  dualperformance: {
    onSwitchOut(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== "Paletta")
        return;
    ...
    onStart(pokemon) {
      ...
      if (!this.effectState.heroMessageDisplayed && pokemon.species.forme === "Hero") {
        ...
        if (pokemon.species.forme === "Pirouette")
          this.add("-message", `${pokemon.name} underwent a rhythmic transformation!`);
        else
          this.add("-message", `${pokemon.name} underwent a melodic transformation!`);
      }
    },
    flags: { failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, failskillswap: 1, cantsuppress: 1, notransform: 1 },
  ...
  zoomies: {
    onModifyPriority(priority, pokemon, target, move) {
      if (move.flags["contact"] && this.randomChance(3, 10)) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${pokemon.name}|Cumon step it up!!!!!`);
        return priority + 1;
      }
    },
    flags: {},
  ...
  sweatypalms: {
    ...
    onSetStatus(status, target, source, effect) {
      if (status.id !== "brn" && status.id !== "psn")
        return;
      if (effect?.status) {
  ...
  footballofruin: {
    ...
    onAnyModifySTAB(stab, source, target, move) {
      if (source.hasAbility("footballofruin"))
        return;
    ...
    onSourceBasePower(basePower, attacker, defender, move) {
      if (attacker.hasAbility("footballofruin"))
        return;
  ...
  supersaiyan: {
    ...
    onAnyTryPrimaryHit(target, source, move) {
      ...
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${source.name}|You look strong. Let's fight!`);
  ...
  unimpressivenessofruin: {
    ...
    onAnyModifySTAB(stab, source, target, move) {
      if (source.hasAbility("unimpressivenessofruin"))
        return;
      if (move.forceSTAB || source.hasType(move.type)) {
        if (stab === 0.75) {
          return 1;
  ...
  doubledown: {
    ...
    onUpdate(pokemon) {
      for (const target of this.getAllActive()) {
        if (!target.hasAbility("doubledown") && !target.volatiles["ability:simple"])
          target.addVolatile("ability:simple");
      }
    },
    onEnd() {
      for (const target of this.getAllActive()) {
        if (target.volatiles["ability:simple"])
          target.removeVolatile("ability:simple");
      }
    },
    flags: {},
    name: "Double Down"
  ...
  sketcher: {
    ...
    onDeductPP(target, source) {
      ...
      if (moveSlot === null || target.hasMove(moveSlot))
        return;
      this.attrLastMove("[still]");
  ...
  grumpossaurus: {
    ...
    onResidualSubOrder: 2,
    onResidual(pokemon) {
      if (pokemon.volatiles["grumpossaurus"])
        pokemon.removeVolatile("grumpossaurus");
    ...
    condition: {
      ...
      onModifyAtkPriority: 5,
      onModifyAtk(atk, pokemon) {
        if (this.effectState.boostedStat !== "atk" || pokemon.ignoringAbility())
          return;
        this.debug("Protosynthesis atk boost");
        return this.chainModify([5325, 4096]);
      },
      onModifyDefPriority: 6,
      onModifyDef(def, pokemon) {
        if (this.effectState.boostedStat !== "def" || pokemon.ignoringAbility())
          return;
        this.debug("Protosynthesis def boost");
        return this.chainModify([5325, 4096]);
      },
      onModifySpAPriority: 5,
      onModifySpA(spa, pokemon) {
        if (this.effectState.boostedStat !== "spa" || pokemon.ignoringAbility())
          return;
        this.debug("Protosynthesis spa boost");
        return this.chainModify([5325, 4096]);
      },
      onModifySpDPriority: 6,
      onModifySpD(spd, pokemon) {
        if (this.effectState.boostedStat !== "spd" || pokemon.ignoringAbility())
          return;
        this.debug("Protosynthesis spd boost");
        return this.chainModify([5325, 4096]);
      },
      onModifySpe(spe, pokemon) {
        if (this.effectState.boostedStat !== "spe" || pokemon.ignoringAbility())
          return;
        this.debug("Protosynthesis spe boost");
        return this.chainModify(1.5);
      },
  ...
  chemicalimbalance: {
    onUpdate(pokemon) {
      ...
      let changes = 0;
      if (pokemon.previousTypes.length !== pokemon.types.length)
        changes += Math.abs(pokemon.previousTypes.length - pokemon.types.length);
      for (const type of pokemon.types) {
        if (!pokemon.previousTypes.includes(type))
          changes++;
      }
      if (changes > 0)
        this.boost({ atk: 2 * changes }, pokemon, pokemon, null, false, true);
      pokemon.previousTypes = pokemon.types;
    },
    flags: {},
  ...
  fruitloop: {
    ...
    onEatItem(item, pokemon) {
      if (!pokemon.berryCount)
        pokemon.berryCount = 0;
      if (pokemon.berryCount === 4)
        return;
    ...
    onResidual(pokemon) {
      if (pokemon.berryCount && pokemon.berryCount !== 0 && !pokemon.item)
        pokemon.berryCount = 0;
    },
    flags: {},
    name: "Fruit Loop"
    ...
    onStart(pokemon) {
      ...
      if (fainted.length > 0) {
        ...
        for (const ally of fainted) {
          this.add(`c:|${Math.floor(Date.now() / 1e3)}|${ally.name}|${msgs[i]}`);
          i++;
  ...
  speeeen: {
    onUpdate(pokemon) {
      if (pokemon.volatiles["confusion"]) {
        if (!pokemon.volatiles["speeeen"])
          pokemon.addVolatile("speeeen");
      } else
        pokemon.removeVolatile("speeeen");
    },
    condition: {
      noCopy: true,
      onStart(pokemon, source, effect) {
  ...
  girlofruin: {
    ...
    onUpdate(pokemon) {
      for (const target of this.getAllActive()) {
        if (!target.hasAbility("girlofruin") && !target.volatiles["ability:minus"])
          target.addVolatile("ability:minus");
      }
    },
    onEnd() {
      for (const target of this.getAllActive()) {
        if (target.volatiles["ability:minus"])
          target.removeVolatile("ability:minus");
      }
    },
    flags: {},
    name: "Girl of Ruin"
  ...
  hotpotato: {
    onSwitchOut(pokemon) {
      if (pokemon.adjacentFoes().length === 0)
        return;
      const target = this.sample(pokemon.adjacentFoes());
      if (pokemon.getAbility().flags["failskillswap"])
        return;
  ...
  coinflipmechanics: {
    onStart(pokemon) {
      this.add(`c:|${Math.floor(Date.now() / 1e3)}|${pokemon.name}|Let's go gambling!`);
    },
  ...
  healthyself: {
    ...
    onDamagingHit(damage, target, source, move) {
      if (!target.hp) {
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${source.name}|What have I done... I must cleanse my soul.`);
  ...
  pranknician: {
    ...
    onModifyMove(move, attacker) {
      if (move.category === "Status") {
        ...
        if (physicalTypes.includes(move))
  ...
  dragonsbody: {
    onSourceDamagingHit(damage, target, source, move) {
      if (move.type !== "Dragon" || target.hasAbility("shielddust") || target.hasItem("covertcloak"))
        return;
      if (this.randomChance(3, 10)) {
        if (target.lastMove)
          target.addVolatile("disable");
  ...
  icyice: {
    ...
    onCriticalHit(target, type, move) {
      if (!target)
        return;
      if (move.category === "Physical" && target.species.id !== "froscue")
        return;
      if (move.category === "Special" && target.species.id !== "froscuenoiceice")
        return;
    ...
    onAfterMoveSecondary(target, source, move) {
      if (source && source !== target && move?.flags["contact"]) {
        ...
        for (let i = 0; i < source.moveSlots.length; i++) {
          ...
          if (moveSlot === null || moveSlot.category !== "Status")
            continue;
          this.attrLastMove("[still]");
          const learnedMove = {
            move: moveSlot,
        ...
        if (success)
          this.add(`c:|${Math.floor(Date.now() / 1e3)}|${target.name}|Yoink!`);
      }
    }
  ...
  commandthesand: {
    ...
    onWeatherChange(pokemon) {
      if (!pokemon.isActive || pokemon.baseSpecies.baseSpecies !== "Chompgiri" || pokemon.transformed)
        return;
      if (!pokemon.hp)
        return;
  ...
  ironify: {
    onUpdate(pokemon) {
      for (const target of this.getAllActive()) {
        if (target !== pokemon && !target.volatiles["ability:ironified"])
          target.addVolatile("ability:ironified");
      }
    },
    onEnd() {
      for (const target of this.getAllActive()) {
        if (target.volatiles["ability:ironified"])
          target.removeVolatile("ability:ironified");
      }
    },
    flags: {},
    name: "Ironify"
  ...
  ironified: {
    onModifyType(move, pokemon) {
      ...
      if (!noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized) && this.randomChance(3, 10)) {
        move.type = "Steel";
        move.typeChangerBoosted = this.effect;
      }
    },
