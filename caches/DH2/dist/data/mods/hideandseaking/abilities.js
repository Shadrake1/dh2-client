...
const Abilities = {
  ...
  defeatist: {
    ...
    shortDesc: "While this Pokemon has 1/2 or less of its max HP, its Attack and Sp. Atk are doubled.",
    flags: {},
    name: "Defeatist",
    rating: -1,
    num: 129
  ...
  solarpower: {
    ...
    onAnySetWeather(target, source, weather) {
      const strongWeathers = ["desolateland", "primordialsea", "deltastream"];
      if (this.field.getWeather().id === "sunnyday" && !strongWeathers.includes(weather.id))
        return false;
    },
    onEnd(pokemon) {
      if (this.field.weatherState.source !== pokemon)
