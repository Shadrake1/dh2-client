...
const Moves = {
  ...
  // Eternal Magikarp
  callforfamily: {
    // todo: figure out how to make team preview ask for order if you have this move
    num: -129,
    accuracy: true,
    basePower: 0,
    category: "Status",
  ...
  crystallize: {
    ...
    onHit(target, source, move) {
      ...
      if (source.volatiles["crystallize"])
        return success;
    },
  ...
  soulsplittingslices: {
    ...
    onPrepareHit(target, source) {
      ...
      if (target.types[1])
        targetTypes.push(target.types[1]);
    ...
    onEffectiveness(typeMod, target, type, move) {
      const i = move.hit - 1;
      if (type == move.targetTypes[i])
        return this.dex.getEffectiveness(move.type, type);
      else
        return 0;
    },
    secondary: null,
  ...
  kleptomania: {
    ...
    basePowerCallback(pokemon, target, move) {
      ...
      if (pokemon.timesStolen)
        bp += 150 * pokemon.timesStolen;
      this.debug("BP: " + bp);
      return bp;
    },
    category: "Physical",
  ...
  continuoussteps: {
    num: -133,
    accuracy: 50,
    // unmodifiable accuracy implemented in eternalpokemon/scripts.ts
    basePower: 50,
    category: "Physical",
    ...
    condition: {
      ...
      onAfterMove(source, target, move) {
        if (move && move.category == "Status" && source && source.hp) {
          if (!this.canSwitch(source.side) || source.forceSwitchFlag || source.beingCalledBack || source.isSkyDropped())
            return;
          if (source.volatiles["commanding"] || source.volatiles["commanded"])
            return;
