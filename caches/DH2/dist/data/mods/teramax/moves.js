  ...
  tarshot: {
    ...
    onHit(target) {
      ...
      if (target.newlySwitched || this.queue.willMove(target))
        return;
      if (target.hasAbility("flashfire") || target.hasAbility("waterbubble") || target.hasAbility("thermalexchange") || target.hasAbility("waterveil") || target.hasAbility("thickfat") || target.hasAbility("steamengine") || target.hasAbility("wellbakedbody") || target.hasAbility("heatproof") || target.hasAbility("purifyingsalt")) {
  ...
  eeriespell: {
    ...
    secondary: {
      ...
      onHit(target, source, move) {
        if (!target.hp)
          return;
        let foeMove = target.lastMove;
        if (!foeMove || move.isZ)
          return;
        if (foeMove.isMax && foeMove.baseMove)
          foeMove = this.dex.moves.get(foeMove.baseMove);
        if (source.isActive)
    ...
    self: {
      onHit(source) {
        ...
        if (this.field.clearTerrain())
          success = true;
        if (success)
          !!this.heal(this.modify(source.maxhp, 0.25));
        return success;
      }
    },
    secondary: null,
