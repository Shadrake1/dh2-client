...
const Abilities = {
    ...
    onDamage(damage, target, source, effect) {
      if (effect.effectType !== "Move" && target.hp >= target.maxhp) {
        if (effect.effectType === "Ability")
          this.add("-activate", target, "ability: " + effect.name);
        return false;
      }
    },
  ...
  battlebondcharizard: {
    onSourceAfterFaint(length, target, source, effect) {
      if (effect?.effectType !== "Move")
        return;
      if (source.abilityState.battleBondcharizardTriggered)
        return;
  ...
  unseenfist: {
    onModifyMove(move) {
      if (this.effectState.unseenFist)
        return;
    ...
    onAfterMoveSecondarySelf(source, target, move) {
      if (!move || !target || source.switchFlag === true)
        return;
      if (target !== source && move.flags["contact"] && (target.volatiles["protect"] || target.volatiles["banefulbunker"] || target.volatiles["kingsshield"] || target.volatiles["spikyshield"] || target.side.getSideCondition("matblock") || target.volatiles["silktrap"] || target.volatiles["burningbulwark"])) {
  ...
  intimidate: {
    onStart(pokemon) {
      let activated = false;
      for (const target of pokemon.adjacentFoes()) {
        if (this.effectState.intim && this.gameType !== "doubles")
          return;
        if (!activated) {
