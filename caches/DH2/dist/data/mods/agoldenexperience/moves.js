...
const Moves = {
  ...
  frostbite: {
    ...
    basePower: 0,
    category: "Status",
    desc: "The Pok\xE9mon at the user's position steals some of the target's maximum HP at the end of each turn. Damage begins at 1/16, rounded down, and increases each turn like Toxic. If Big Root is held by the recipient, the HP recovered is 1.3x normal, rounded half down. If the target uses Baton Pass, the replacement will continue being leeched. If the target switches out, the effect ends.",
    ...
    condition: {
      ...
      onResidual(pokemon2) {
        ...
        for (const target of this.getAllActive()) {
          if (pokemon2.volatiles["frostbite"]) {
            const damage = this.damage(this.clampIntRange(pokemon2.baseMaxhp / 16, 1) * this.effectState.stage, pokemon2, target);
            if (damage) {
              this.heal(damage, target, pokemon2);
            }
          }
  ...
  indomitablespirit: {
    ...
    flags: { contact: 1, protect: 1, mirror: 1 },
    basePowerCallback(pokemon2, target, move) {
      if (pokemon2.moveLastTurnResult === false)
        return move.basePower * 2;
      if (pokemon2.volatiles["indomitablespirit"].boost === "lastMoveResisted")
        return move.basePower * 2;
      return move.basePower;
    },
    onPrepareHit: function(target, source) {
    ...
    condition: {
      // this is *not* meant to be set as part of the move; partially defined in scripts.ts!
  ...
  contrariety: {
    ...
    onHitField() {
      ...
      for (const pokemon2 of this.getAllActive()) {
        if (pokemon2.ability === "truant" || pokemon2.ability === "contrary" || pokemon2.getAbility().flags["cantsuppress"])
          continue;
        const oldAbility = pokemon2.setAbility("contrary");
        if (oldAbility)
          this.add("-ability", pokemon2, "Contrary", "[from] move: Contrary");
        success = true;
      }
      return success;
    },
    ...
    condition: {
      ...
      onFieldStart(field, source, effect) {
        if (effect?.effectType === "Ability") {
          ...
          this.add("-message", "Fighting-type moves used by grounded Pok\xE9mon won't miss.");
  ...
  naturepower: {
    ...
    onTryHit(target, pokemon2) {
      ...
      this.actions.useMove(move, pokemon2, target);
      return null;
    }
  },
  terrainpulse: {
  ...
  camouflage: {
    ...
    onHit(target) {
      ...
      if (target.hasItem("identitycard"))
        return false;
  ...
  ragingfury: {
    ...
    basePowerCallback(pokemon2, target, move) {
      ...
      if (pokemon2.volatiles["ragingfury"] && pokemon2.volatiles["ragingfury"].hitCount) {
        bp -= 30 * pokemon2.volatiles["ragingfury"].hitCount;
      }
      if (pokemon2.status !== "slp")
        pokemon2.addVolatile("ragingfury");
  ...
  guillotine: {
    ...
    onAfterMoveSecondarySelf(pokemon2, target, move) {
      if (!target || target.fainted || target.hp <= 0)
        this.boost({ atk: 1 }, pokemon2, pokemon2, move);
    }
  },
  horndrill: {
    inherit: true,
  ...
  lightningassault: {
    ...
    accuracy: 100,
    basePower: 0,
    basePowerCallback(pokemon2, target) {
      let ratio = Math.floor(pokemon2.getStat("spe") / target.getStat("spe"));
      if (!isFinite(ratio))
        ratio = 0;
  ...
  awakening: {
    ...
    onHit(pokemon2) {
      for (const target of pokemon2.side.foe.active) {
        ...
        this.add("-message", pokemon2.name + "'s Awakening revealed the move " + temp.move + "!");
      }
    },
    onPrepareHit: function(target, source) {
      this.attrLastMove("[still]");
  ...
  braveblade: {
    ...
    onModifyMove(move, pokemon2, target) {
      if (!target)
        return;
      const atk = pokemon2.getStat("atk", false, true);
      const spa = pokemon2.getStat("spa", false, true);
      const def = target.getStat("def", false, true);
      const spd = target.getStat("spd", false, true);
      const physical = Math.floor(Math.floor(Math.floor(Math.floor(2 * pokemon2.level / 5 + 2) * 90 * atk) / def) / 50);
      const special = Math.floor(Math.floor(Math.floor(Math.floor(2 * pokemon2.level / 5 + 2) * 90 * spa) / spd) / 50);
  ...
  houndshowl: {
    ...
    beforeTurnCallback(pokemon2) {
      for (const side of this.sides) {
        if (side === pokemon2.side)
          continue;
        side.addSideCondition("houndshowl", pokemon2);
        const data = side.getSideConditionData("houndshowl");
        if (!data.sources) {
          data.sources = [];
        }
        data.sources.push(pokemon2);
      }
    },
    onModifyMove(move, source, target) {
      if (target?.beingCalledBack)
    ...
    condition: {
      duration: 1,
      onBeforeSwitchOut(pokemon2) {
        this.debug("Hound's Howl start");
        let alreadyAdded = false;
        pokemon2.removeVolatile("destinybond");
        for (const source of this.effectState.sources) {
          if (!source.isAdjacent(pokemon2) || !this.queue.cancelMove(source) || !source.hp)
            continue;
          if (!alreadyAdded) {
            this.add("-activate", pokemon2, "move: Hound's Howl");
            alreadyAdded = true;
          }
          ...
          this.actions.runMove("houndshowl", source, source.getLocOf(pokemon2));
        }
      }
    },
    onPrepareHit: function(target, source) {
  ...
  junglehealing: {
    inherit: true,
    onHit(pokemon2) {
      const success = !!this.heal(this.modify(pokemon2.maxhp, 0.33));
      return pokemon2.cureStatus() || success;
    },
  ...
  monkeybusiness: {
    ...
    flags: { contact: 1, protect: 1, mirror: 1 },
    onModifyMove(move, pokemon2) {
      if (pokemon2.getStat("spa", false, true) > pokemon2.getStat("atk", false, true))
    ...
    onHit(pokemon2) {
      if (pokemon2.item && pokemon2.getItem().isBerry)
        pokemon2.eatItem(true);
      if (!pokemon2.lastItem)
        return false;
      const item = pokemon2.lastItem;
      pokemon2.lastItem = "";
      this.add("-item", pokemon2, this.dex.items.get(item), "[from] move: Monkey Business");
  ...
  shattering: {
    ...
    condition: {
      onUpdate(pokemon2) {
        ...
        this.add("-enditem", pokemon2, item.name, "[from] move: Shattering");
        this.runEvent("AfterUseItem", pokemon2, null, null, item);
        pokemon2.removeVolatile("fling");
      }
    },
    secondary: null,
    target: "normal",
  ...
  tidyup: {
    ...
    onHit(pokemon2) {
      ...
      const sides = [pokemon2.side, ...pokemon2.side.foeSidesWithConditions()];
      ...
      if (success)
        this.add("-activate", pokemon2, "move: Tidy Up");
      return !!this.boost({ atk: 1, spe: 1, accuracy: 1 }, pokemon2, pokemon2, null, false, true) || success;
    }
  },
  hyperdrill: {
    inherit: true,
  ...
  lastrespects: {
    inherit: true,
    basePowerCallback(pokemon2, target, move) {
      return 50 + 25 * pokemon2.side.totalFainted;
    },
    onModifyMove(move, pokemon2) {
      if (pokemon2.getStat("spa", false, true) > pokemon2.getStat("atk", false, true))
        move.category = "Special";
  ...
  ragefist: {
    inherit: true,
    basePowerCallback(pokemon2) {
      return Math.min(200, 50 + 25 * pokemon2.timesAttacked);
    },
    ...
    onModifyType(move, pokemon2) {
      for (const target of pokemon2.side.foe.active) {
        ...
        if (this.dex.getEffectiveness(type1, target) < this.dex.getEffectiveness(type2, target)) {
          ...
          if (pokemon2.hasType("Ground") && !pokemon2.hasType("Grass")) {
            move.type = "Ground";
          }
        }
      }
  ...
  clusterexplosion: {
    ...
    onPrepareHit(target, pokemon2, move) {
      this.attrLastMove("[still]");
      this.add("-anim", pokemon2, "Splintered Stormshards", pokemon2);
    },
  ...
  thunderway: {
    ...
    onHit(pokemon2) {
      const success = !!this.heal(this.modify(pokemon2.maxhp, 0.75));
  ...
  fieryfire: {
    ...
    onHit(pokemon2) {
      pokemon2.addVolatile("flashfire");
    },
    onEnd(pokemon2) {
  ...
  boulderbuilding: {
    ...
    onHit(target, source, move) {
      let success = false;
      if (!target.volatiles["substitute"] || move.infiltrates)
        success = !!this.boost({ def: 1 }, source, source, this.dex.getActiveMove("Boulder Building"));
      const removeTarget = [
        "reflect",
        "lightscreen",
        "auroraveil",
  ...
  alienmetal: {
    ...
    condition: {
      onStart(pokemon2, source, effect) {
        this.add("-start", pokemon2, "Alien Metal");
      },
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Alien Metal", "[silent]");
      }
    },
    onPrepareHit: function(target, source) {
      this.attrLastMove("[still]");
  ...
  dig: {
    ...
    condition: {
      ...
      onImmunity(type, pokemon2) {
        if (type === "sandstorm" || type === "hail" || type === "everlastingwinter")
          return false;
      },
  ...
  moonlight: {
    ...
    onHit(pokemon2) {
      ...
      const success = !!this.heal(this.modify(pokemon2.maxhp, factor));
      if (!success) {
        this.add("-fail", pokemon2, "heal");
        return this.NOT_FAIL;
      }
      return success;
    }
  ...
  solarbeam: {
    ...
    onBasePower(basePower, pokemon2, target) {
      ...
      if (weakWeathers.includes(pokemon2.effectiveWeather())) {
  ...
  nightmare: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon2) {
        if (pokemon2.status !== "slp" && !pokemon2.hasAbility("comatose") && !this.field.getPseudoWeather("endlessdream")) {
          return false;
        }
        this.add("-start", pokemon2, "Nightmare");
      },
      onResidualOrder: 9,
      onResidual(pokemon2) {
