...
const Abilities = {
    ...
    onResidual(pokemon) {
      ...
      if (pokemon.activeTurns) {
        for (const target of pokemon.side.foe.active) {
          if (!target || !target.isAdjacent(pokemon) || !target.isGrounded())
            continue;
          if (!activated) {
  ...
  mistymountain: {
    ...
    onModifyType(move, pokemon) {
      ...
      if (move.type === "Rock" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && !(move.name === "Tera Blast" && pokemon.terastallized)) {
        move.type = "Ice";
  ...
  sundownswitch: {
    ...
    onBeforeMove(attacker, defender, move) {
      if (attacker.species.baseSpecies !== "Cacturne" || attacker.transformed)
        return;
      if (move.type !== "Grass" && move.type !== "Dark")
        return;
  ...
  forgery: {
    ...
    onStart(pokemon) {
      if (pokemon.species.name !== "Zoroark-Mega")
        return;
      pokemon.addVolatile("forgery");
      let i;
      for (i = pokemon.side.pokemon.length - 1; i > pokemon.position; i--) {
        if (!pokemon.side.pokemon[i] || pokemon.side.pokemon[i].fainted || !pokemon.side.pokemon[i].item || this.dex.items.get(pokemon.side.pokemon[i].item).zMove || this.dex.items.get(pokemon.side.pokemon[i].item).megaStone)
          continue;
        break;
      }
  ...
  clairvoyance: {
    ...
    onBeforeMove(source, target, move) {
      if (move && move.type === "Psychic" && source.hasAbility("clairvoyance") && source.side.addSlotCondition(source, "clairvoyance")) {
  ...
  northwind: {
    ...
    onResidual(pokemon) {
      ...
      if (pokemon.activeTurns) {
        for (const target of pokemon.side.foe.active) {
          if (!target || !target.isAdjacent(pokemon) || target.hasType("Ice"))
            continue;
          if (!activated) {
            this.add("-ability", pokemon, "Web Weaver", "boost");
            activated = true;
  ...
  shadowtag: {
    ...
    onFoeSwitchOut(source, target) {
      for (const target2 of source.side.foe.active) {
  ...
  grasspelt: {
    inherit: true,
    onStart(pokemon) {
      if (!this.field.setTerrain("grassyterrain") && this.field.isTerrain("grassyterrain") && pokemon.isGrounded()) {
        this.add("-activate", pokemon, "ability: Grass Pelt");
      }
    },
    onTerrainChange(pokemon) {
      if (pokemon === this.field.weatherState.source)
        return;
  ...
  faithfulcompanion: {
    onUpdate(pokemon) {
      ...
      if (!ally || pokemon.transformed || pokemon.baseSpecies.baseSpecies !== "Parrotnair" || ally.baseSpecies.baseSpecies !== "Piratcy") {
        return;
      }
    },
