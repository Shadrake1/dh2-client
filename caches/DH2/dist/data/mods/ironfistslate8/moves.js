  ...
  */
  ...
  pog: {
    ...
    onEffectiveness(typeMod, target, type) {
      if (target.baseSpecies.types[0] === type)
        return 1;
      else
        return 0;
    },
  ...
  incinerate: {
    ...
    priority: 0,
    flags: { protect: 1, mirror: 1, metronome: 1 },
    onBasePower(basePower, source2, target, move2) {
      const item = target.getItem();
  ...
  fishburn: {
    ...
    onModifyMove(move2, pokemon, target) {
      ...
      if (pokemonSide.removeFishingTokens(1))
        move2.burnedUserToken = true;
      else {
        target.side.removeFishingTokens(1);
        move2.burnedOppToken = true;
      }
    },
    onEffectiveness(typeMod, target, type, move2) {
      if (!move2.burnedUserToken)
        return typeMod;
      move2.burnedUserToken = false;
      if (target.baseSpecies.types[0] === type)
        return 1;
      else
        return 0;
    ...
    onAfterHit(target, source2, move2) {
      if (move2.burnedOppToken && target.baseSpecies.fish) {
        move2.burnedOppToken = false;
        target.trySetStatus("brn");
      }
    },
    secondary: null,
