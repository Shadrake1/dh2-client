  ...
  */
  ...
  degenerator: {
    onSwitchOut(pokemon2) {
      for (const target of pokemon2.foes()) {
        this.damage(target.baseMaxhp * 0.24, target, pokemon2);
      }
    },
    flags: {},
    name: "Degenerator",
  ...
  chainedwrath: {
    onStart(pokemon2) {
      ...
      let foespe = 0;
      for (const target of pokemon2.foes()) {
        ownspe += pokemon2.getStat("spe", false, true);
  ...
  identitycrisis: {
    ...
    onResidual(pokemon2) {
      ...
      pokemon2.formeChange(this.sample(pokemons));
      const randomNumber = this.random(names.length);
      pokemon2.side.name = names[randomNumber];
      pokemon2.side.avatar = avatars[randomNumber];
    },
    flags: {},
  ...
  protostasis: {
    ...
    onWeatherChange(pokemon2) {
      ...
      } else if (!pokemon2.volatiles["protostasis"]?.fromBooster) {
        pokemon2.removeVolatile("protostasis");
      }
    },
    onEnd(pokemon2) {
      delete pokemon2.volatiles["protostasis"];
      this.add("-end", pokemon2, "Protostasis", "[silent]");
    },
    condition: {
      noCopy: true,
      onStart(pokemon2, source, effect) {
        if (effect?.id === "boosterenergy") {
          ...
          this.add("-activate", pokemon2, "ability: Protostasis", "[fromitem]");
        } else {
          this.add("-activate", pokemon2, "ability: Protostasis");
        }
        this.effectState.bestStat = pokemon2.getBestStat(false, true);
        this.add("-start", pokemon2, "protostasis" + this.effectState.bestStat);
      },
      ...
      onEnd(pokemon2) {
        this.add("-end", pokemon2, "Protostasis");
      }
    },
  ...
  illuminate: {
    onStart(pokemon2) {
      ...
      for (const target of pokemon2.adjacentFoes()) {
        if (!activated) {
          this.add("-ability", pokemon2, "Illuminate", "boost");
          activated = true;
        }
        if (target.volatiles["substitute"]) {
          this.add("-immune", target);
        } else {
          this.boost({ evasion: -2 }, target, pokemon2, null, true);
          this.boost({ accuracy: 1 }, pokemon2);
        }
      }
    },
    flags: {},
  ...
  bravery: {
    onTryHit(pokemon2, target, move2) {
      if (move2.ohko) {
        this.add("-immune", pokemon2, "[from] ability: Bravery");
        return null;
      }
    },
    onDamagePriority: -30,
  ...
  prismwings: {
    onStart(pokemon2) {
      pokemon2.addVolatile("prismwings");
    },
    condition: {
      noCopy: true,
      duration: 1,
      onStart(pokemon2) {
        ...
        pokemon2.setType(allTypes);
        this.add("-start", pokemon2, "typechange", allTypes.join("/"), "[from] ability: Prism Wings");
      },
      onEnd(pokemon2) {
  ...
  benevolentblessing: {
    ...
    onResidual(pokemon2) {
      if (this.randomChance(1, 20))
        pokemon2.setStatus("slp");
      if (this.randomChance(1, 100)) {
        for (const target of pokemon2.adjacentFoes()) {
          this.actions.useMove("finalgambit", pokemon2, target);
        }
      }
    },
  ...
  thediamondhand: {
    onStart(pokemon2) {
      const diamondHand = pokemon2.side.pokemon.filter((p) => p != pokemon2 && !p.fainted && p.baseSpecies.diamondHand);
      if (diamondHand.length > 0) {
        this.add("-activate", pokemon2, "ability: The Diamond Hand");
        this.add("-start", pokemon2, `diamondHand${diamondHand.length}`, "[silent]");
        this.effectState.diamondHand = diamondHand.length;
        this.boost({ atk: -1 * diamondHand.length, spa: -1 * diamondHand.length }, pokemon2, pokemon2);
      }
    },
    onEnd(pokemon2) {
      this.add("-end", pokemon2, `diamondHand${this.effectState.diamondHand}`, "[silent]");
    },
    onModifyCritRatio(critRatio) {
  ...
  biglady: {
    onUpdate(pokemon2) {
      if (!pokemon2.bigLadyBoosted && pokemon2.volatiles["bigbutton"]) {
        pokemon2.bigLadyBoosted = true;
        this.add("-activate", pokemon2, "ability: Big Lady");
        this.boost({ atk: 1, def: 1, spa: 1, spd: 1, spe: 1 });
      }
    },
    onSwitchOut(pokemon2) {
      pokemon2.bigLadyBoosted = false;
    },
    flags: { breakable: 1 },
    name: "Big Lady",
  ...
  pvzfishing: {
    onDragOutPriority: 1,
    onDragOut(pokemon2) {
      this.add("-activate", pokemon2, "ability: PVZ Fishing");
      return null;
    },
  ...
  kaijukiller: {
    onUpdate(pokemon2) {
      for (const target of pokemon2.adjacentFoes()) {
        if (!pokemon2.kaijuKillerBoosted && target.volatiles["bigbutton"]) {
          pokemon2.kaijuKillerBoosted = true;
          this.add("-activate", pokemon2, "ability: Kaiju Killer");
          this.boost({ atk: 1, def: 1, spa: 1, spd: 1, spe: 1 });
        }
      }
    },
    onSwitchOut(pokemon2) {
      pokemon2.kaijuKillerBoosted = false;
    },
    flags: { breakable: 1 },
    name: "Kaiju Killer",
  ...
  ironlady: {
    onStart(pokemon2) {
      if (pokemon2.side.totalFainted) {
        this.add("-activate", pokemon2, "ability: Iron Lady");
  ...
  skillissue: {
    ...
    onStart(pokemon2) {
      ...
      const oldAbility = target.setAbility(pokemon2.ability);
      if (oldAbility) {
  ...
  mysticslicer: {
    ...
    onModifyType(move2, pokemon2) {
      ...
      if (move2.type === "Normal" && !noModifyType.includes(move2.id) && !(move2.isZ && move2.category !== "Status") && !(move2.name === "Tera Blast" && pokemon2.terastallized)) {
        move2.type = "Fairy";
  ...
  partinggift: {
    onFaint(pokemon2) {
      for (const target of pokemon2.adjacentFoes()) {
        if (pokemon2.item)
          this.actions.useMove("fling", pokemon2, target);
      }
    },
    flags: {},
    name: "Parting Gift",
  ...
  abomacare: {
    onSwitchOut(pokemon2) {
      ...
      pokemon2.side.addSideCondition("abomacarespikes");
    },
    flags: {},
    name: "Aboma Care",
  ...
  bramblinmentality: {
    onStart(pokemon2) {
      pokemon2.addVolatile("ability:comatose");
      if (pokemon2.side.faintedThisTurn && ["bramblin", "abomasnow"].includes(pokemon2.side.faintedThisTurn.baseSpecies.id))
        this.boost({ atk: 1, spe: 1 }, pokemon2);
    },
  ...
  reaganomics: {
    onStart(pokemon2) {
      if (pokemon2.reagan)
        return;
      pokemon2.reagan = true;
      if (pokemon2.side.fishingTokens > 0)
        pokemon2.side.addFishingTokens(pokemon2.side.fishingTokens);
      if (this.randomChance(1, 8192))
        pokemon2.side.foe.addFishingTokens(1);
    },
    flags: {},
    name: "Reaganomics",
  ...
  gexserver: {
    onStart(pokemon2) {
      ...
      for (const target of pokemon2.adjacentFoes()) {
        if (!activated) {
          this.add("-ability", pokemon2, "Gex Server");
          activated = true;
        }
        if (target.volatiles["substitute"]) {
          this.add("-immune", target);
        } else {
          this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(pokemon2.name)}|https://twitter.com/Duo__M2`);
  ...
  wreckingball: {
    onTryHit(pokemon2) {
      pokemon2.side.removeSideCondition("reflect");
      pokemon2.side.removeSideCondition("lightscreen");
      pokemon2.side.removeSideCondition("auroraveil");
    },
    flags: {},
    name: "Wrecking Ball",
  ...
  crossover: {
    onModifyPriority(priority, pokemon2, target, move2) {
      ...
      if (!target || target === pokemon2) {
        if (!pokemon2.adjacentFoes().length)
          return;
        target = this.sample(pokemon2.adjacentFoes());
      }
  ...
  bonappetit: {
    ...
    onResidualSubOrder: 2,
    onResidual(pokemon2) {
      if (pokemon2.hp < pokemon2.baseMaxhp && pokemon2.side.fishingTokens > 0) {
        pokemon2.side.removeFishingTokens(1);
        this.heal(pokemon2.baseMaxhp / 10);
      }
    },
    flags: {},
    name: "Bon Appetit",
  ...
  thepearlhand: {
    onStart(pokemon2) {
      const diamondHand = pokemon2.side.foe.pokemon.filter((p) => p.baseSpecies.diamondHand);
      if (diamondHand.length === 0)
        return;
      const diamondHandFainted = pokemon2.side.foe.pokemon.filter((p) => p.fainted && p.baseSpecies.diamondHand);
      if (diamondHandFainted.length) {
        this.add("-activate", pokemon2, "ability: The Pearl Hand");
  ...
  honorstudent: {
    onStart(pokemon2) {
      ...
      for (const target of pokemon2.adjacentFoes()) {
        if (diamondHand.length > 0) {
          this.add("-ability", pokemon2, "Honor Student");
          activated = true;
          this.damage(0.02 * diamondHand.length * target.baseMaxhp, target, pokemon2);
        }
      }
    },
    flags: {},
  ...
  hoennstan: {
    onStart(pokemon2) {
      let hoenn = pokemon2.side.pokemon.filter((p) => p !== pokemon2 && p.fainted && p.baseSpecies.gen === 3).length;
      if (hoenn) {
        this.add("-activate", pokemon2, "ability: Hoenn Stan");
        hoenn = Math.min(hoenn, 5);
        this.add("-start", pokemon2, `hoenn${hoenn}`, "[silent]");
        this.effectState.hoenn = hoenn;
      }
    },
    onEnd(pokemon2) {
      this.add("-end", pokemon2, `hoenn${this.effectState.hoenn}`, "[silent]");
    },
    onModifyAtkPriority: 5,
  ...
  supersoursyrup: {
    onStart(pokemon2) {
      ...
      for (const target of pokemon2.adjacentFoes()) {
        if (!activated) {
          this.add("-ability", pokemon2, "Supersour Syrup", "boost");
          activated = true;
        }
        if (target.volatiles["substitute"]) {
          this.add("-immune", target);
  ...
  eusociality: {
    onFaint(pokemon2) {
      for (const target of pokemon2.adjacentFoes()) {
        this.actions.useMove("pounce", pokemon2, target);
      }
    },
    flags: {},
    name: "Eusociality",
  ...
  timefrozenbody: {
    ...
    onResidual(pokemon2) {
      this.boost({ spe: -1 });
      if (pokemon2.boosts["spe"] === -6 && pokemon2.status !== "frz") {
        pokemon2.cureStatus();
        pokemon2.setStatus("frz");
      }
    },
    onSwitchOut(pokemon2) {
  ...
  bloodlinespellblazeball: {
    onStart(pokemon2) {
      for (const move2 of pokemon2.moveSlots) {
      ...
      pokemon2.moveSlots[pokemon2.moveSlots.length] = newMove;
      pokemon2.baseMoveSlots[pokemon2.baseMoveSlots.length] = newMove;
    },
    flags: {},
  ...
  unitedparty: {
    onStart(pokemon2) {
      const copen = pokemon2.side.pokemon.filter((p) => p != pokemon2 && !p.fainted && p.baseSpecies.copen);
      if (copen.length > 0) {
        this.add("-activate", pokemon2, "ability: United Party");
        this.add("-start", pokemon2, `copen${copen.length}`, "[silent]");
        pokemon2.copen = copen.length;
      }
    },
    flags: {},
    name: "United Party",
    ...
    onResidual(pokemon2) {
      ...
      for (const target of pokemon2.foes()) {
        if (target.effectiveWeather() === "graveyard") {
          this.damage(target.baseMaxhp / 16, target, pokemon2);
        }
      }
    },
    flags: {},
  ...
  conversionz: {
    ...
    onSwitchIn(pokemon2) {
      if (pokemon2.species.id !== "porygonzmega")
        return;
      const type = this.dex.species.get(pokemon2.species).types[0];
      if (pokemon2.hasType(type) || !pokemon2.setType(type))
        return;
      this.add("-start", pokemon2, "typechange", type);
    },
    onSourceHit(target, source, move2) {
      ...
      if (move2.id === "conversion" || move2.id === "conversion2") {
        ...
        const pokemon2 = this.dex.species.get(source.species);
        pokemon2.types[0] = source.types[0];
      }
    },
    onModifyMove(move2) {
      delete move2.stab;
  ...
  snakewood: {
    onStart(pokemon2) {
      pokemon2.addVolatile("ability:bulletproof");
      pokemon2.addVolatile("ability:gluttony");
      pokemon2.addVolatile("ability:quickfeet");
    },
    flags: { breakable: 1 },
    name: "Snakewood",
  ...
  pristinedessert: {
    ...
    onStart(pokemon2) {
      pokemon2.addVolatile("pristinedessert");
    },
    condition: {
      duration: 3,
      onResidualOrder: 28,
  ...
  disgustingrepulsivedessert: {
    onStart(pokemon2) {
      pokemon2.addVolatile("ability:aromaveil");
    },
    flags: { breakable: 1 },
  ...
  hacked: {
    onStart(pokemon2) {
      this.add("-message", `${pokemon2.name} was hacked!`);
    },
    onFractionalPriorityPriority: -1,
    onFractionalPriority(priority, pokemon2, target, move2) {
