...
const Conditions = {
  baseball: {
    ...
    onTryHit(source, target, move) {
      if (move.flags["sound"]) {
        ...
        this.add(`c:|${Math.floor(Date.now() / 1e3)}|${getName(source.name)}|Shut Up\u203C\uFE0F`);
        return null;
      }
    }
  },
  ...
  graveyard: {
    ...
    onWeather(target) {
      if (!target.hasAbility("magicguard") && !target.hasAbility("ghoulgobbler") && !target.hasAbility("rkssystem")) {
