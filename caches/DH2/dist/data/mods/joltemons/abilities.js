...
const Abilities = {
  ...
  merciless: {
    ...
    onModifyCritRatio(critRatio, source, target) {
      if (target && ["psn", "tox", "brn", "frz", "slp", "par"].includes(target.status))
        return 5;
    },
    name: "Merciless",
    rating: 1.5,
  ...
  soullink: {
    ...
    name: "Soul Link",
    shortDesc: "Pok\xE9mon that make contact with this Pok\xE9mon have the Ghost-type added to their existing typings until they switch out (Trick-or-Treat effect).",
    rating: 2.5
  },
  /*
  	soullink: {
  ...
  parentalbond: {
    onPrepareHit(source, target, move) {
      if (move.category === "Status" || move.selfdestruct || move.multihit)
        return;
      if (["endeavor", "seismictoss", "psywave", "nightshade", "sonicboom", "dragonrage", "superfang", "naturesmadness", "bide", "counter", "mirrorcoat", "metalburst"].includes(move.id))
        return;
  ...
  */
  ...
  oblivious: {
    ...
    onImmunity(type, pokemon) {
      if (type === "attract" || type === "trashtalk")
        return false;
    },
    onTryHit(pokemon, target, move) {
