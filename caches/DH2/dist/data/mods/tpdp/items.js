...
const Items = {
  ...
  blitzcharm: {
    ...
    isBerry: true,
    onUpdate(pokemon) {
      if (pokemon.hp <= pokemon.maxhp / 4 || pokemon.hp <= pokemon.maxhp / 2 && pokemon.hasAbility("worrywart") && pokemon.abilityState.worrywart) {
        pokemon.eatItem();
      }
    },
    onEat(pokemon) {
  ...
  capturerope: {
    ...
    onFoeModifyMove(move, pokemon, target) {
      if (!["shadowstich", "poisonlabryinth", "battlemania", "adversewind"].includes(pokemon.ability))
  ...
  ironwillribbon: {
    ...
    onFoeModifyDamage(relayVar, target, source, move) {
      var moveCount = 0;
      for (const move2 in target.moveSlots) {
        if (move2)
          moveCount++;
      }
      if (moveCount === 3)
