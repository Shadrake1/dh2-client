...
const Moves = {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (!pokemon.isGrounded())
          return;
        if (pokemon.types.includes("Darks"))
          return;
  ...
  continue: {
    ...
    onHit(target, source, move) {
      if (!target.setStatus("stp", source, move))
        return false;
  ...
  crosscounter: {
    ...
    onTry(source) {
      if (!source.volatiles["crosscounter"])
        return false;
      if (source.volatiles["crosscounter"].slot === null)
        return false;
    },
    condition: {
      duration: 1,
  ...
  drought: {
    // yoshi note: should be renamed because this messes with aaa's validation and won't allow the drought ability to be banned without crashing the server
    name: "Drought",
  ...
  mirage: {
    ...
    condition: {
      ...
      onTryHit(target, source, move) {
        if (target === source || move.hasBounced || move.category !== "Status" || move.target !== "normal" && move.target !== "foeSide" || move.name !== "Shinigami's Waltz") {
          return;
        }
  ...
  mirrorsreflection: {
    ...
    onTry(source) {
      if (!source.volatiles["mirrorsreflection"])
        return false;
      if (source.volatiles["mirrorsreflection"].slot === null)
        return false;
    },
    condition: {
      duration: 1,
  ...
  moonsreflection: {
    ...
    onTry(source) {
      if (!source.volatiles["moonsreflection"])
        return false;
      if (source.volatiles["moonsreflection"].slot === null)
        return false;
    },
    condition: {
      duration: 1,
  ...
  poisontrap: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        ...
        } else if (pokemon.hasType("Steels") || !pokemon.isGrounded || pokemon.hasItem(["heavydutyboots", "tengugeta"]) || pokemon.hasAbility(["strictdosage"])) {
          return;
    ...
    condition: {
      ...
      onUpdate(pokemon) {
        ...
        if (pokemon.hp) {
          ...
          if (success)
            this.add("-message", `${pokemon.name} blew away the surrounding hazards!`);
