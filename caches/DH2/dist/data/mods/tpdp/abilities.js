...
const Abilities = {
  ...
  adversewind: {
    ...
    onFoeTrapPokemon(pokemon) {
      if (pokemon.hasAbility("aircushion"))
        return;
  ...
  aftermove: {
    ...
    onBasePower(basePower, pokemon, target) {
      for (const target2 of this.getAllActive()) {
        if (target2 === pokemon)
          continue;
        if (pokemon.getStat("spe") > target2.getStat("spe")) {
  ...
  bibliophilia: {
    ...
    onStart(pokemon) {
      ...
      if (!youmascrolls.includes(pokemon.item) || pokemon.species.id !== "extrakosuzu")
        return;
  ...
  harassment: {
    ...
    onAfterBoost(boost, target, source, effect) {
      ...
      if (effect?.name === "Harassment" || effect?.name === "Bronze Mirror")
        return;
      const boostPlus = this.effectState.boosts;
      if (target.adjacentFoes().length == 0)
        return;
    ...
    onAnySwitchIn() {
      if (!this.effectState.boosts || !this.effectState.target)
        return;
  ...
  projection: {
    ...
    onUpdate(pokemon) {
      ...
      const possibleTargets = pokemon.side.foe.active.filter((target2) => !target2.getAbility().isPermanent && !additionalBannedAbilities.includes(target2.ability));
  ...
  secretceremony: {
    ...
    condition: {
      onStart(pokemon) {
        ...
        let newTypes = [];
        if (terrainType)
          newTypes.push(terrainType);
        if (!newTypes.length || pokemon.getTypes().join() === newTypes || !pokemon.setType(newTypes))
          return;
        if (newTypes.length > 1 && newTypes[1] === newTypes[0])
          newTypes.pop();
  ...
  stubborn: {
    name: "Stubborn",
    //TODO - what are the "certain skills" prevented
  ...
  twoofakind: {
    ...
    onPrepareHit(source, target, move) {
      if (move.category === "Status" || move.selfdestruct)
        return;
      if (["dynamaxcannon", "endeavor", "fling", "iceball", "rollout"].includes(move.id))
        return;
