...
const Moves = {
  ...
  swarmingstrike: {
    ...
    onBasePower(basePower, source) {
      ...
      for (const partyMember of source.side.pokemon) {
        if (partyMember !== source && !partyMember.fainted && partyMember.hasType("Bug")) {
          bugCount++;
        }
      }
  ...
  sylvanpowder: {
    ...
    secondary: {
      chance: 33,
      // Might ask submitter to change it to 30% as no other move has 33% trigger chance
      status: "slp"
    },
    target: "normal",
    type: "Grass",
  ...
  firepledge: {
    ...
    basePowerCallback(target, source, move) {
      ...
      if (sourceMove && (sourceMove.type === "Grass" || sourceMove.type === "Water") && sourceMove.category !== "Status" && sourceMove.target === "normal") {
        this.add("-combine");
        return 150;
      }
    ...
    onPrepareHit(target, source, move) {
      for (const action of this.queue.list) {
        ...
        if (action.pokemon.isAlly(source) && partnerMove.category !== "Status" && ["Water", "Grass"].includes(partnerMove.type)) {
    ...
    onModifyMove(move, source) {
      const sourceMove = this.dex.moves.get(move.sourceEffect);
      if (sourceMove && sourceMove.category !== "Status" && sourceMove.target === "normal") {
  ...
  grasspledge: {
    ...
    basePowerCallback(target, source, move) {
      const sourceMove = this.dex.moves.get(move.sourceEffect);
      if (sourceMove && (sourceMove.type === "Water" || sourceMove.type === "Fire") && sourceMove.category !== "Status" && sourceMove.target === "normal") {
        this.add("-combine");
        return 150;
      }
      return move.basePower;
    ...
    onPrepareHit(target, source, move) {
      for (const action of this.queue.list) {
        ...
        const partnerMove = this.dex.moves.get(action.move.id);
        if (action.pokemon.isAlly(source) && partnerMove.category !== "Status" && ["Water", "Fire"].includes(partnerMove.type)) {
          this.queue.prioritizeAction(action, move);
          this.add("-waiting", source, action.pokemon);
          return null;
        }
  ...
  waterpledge: {
    ...
    basePowerCallback(target, source, move) {
      const sourceMove = this.dex.moves.get(move.sourceEffect);
      if (sourceMove && (sourceMove.type === "Fire" || sourceMove.type === "Grass") && sourceMove.category !== "Status" && sourceMove.target === "normal") {
        this.add("-combine");
        return 150;
      }
      return move.basePower;
    ...
    onPrepareHit(target, source, move) {
      for (const action of this.queue) {
        ...
        if (otherMoveUser.isAlly(source) && otherMoveData && (otherMoveData.type === "Fire" || otherMoveData.type === "Grass") && otherMoveData.category !== "Status" && otherMoveData.target === "normal") {
          this.queue.prioritizeAction(action, move);
          this.add("-waiting", source, otherMoveUser);
          return null;
        }
