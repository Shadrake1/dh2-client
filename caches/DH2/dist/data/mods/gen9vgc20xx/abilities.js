...
const Abilities = {
  ...
  //
  mindcontrol: {
    desc: "While this Pok\xE9mon is present, all non-Dark-type Pok\xE9mon are prevented from using the same move twice in a row.",
    ...
    onEnd(pokemon) {
      for (const target of this.getAllActive()) {
        if (target === pokemon)
          continue;
        if (target.hasAbility("mindcontrol"))
          return;
      }
      for (const target of this.getAllActive()) {
  ...
  selfish: {
    ...
    onDamage(damage, target, source, effect) {
      if (effect && effect.effectType !== "Move") {
        const ally = target.side.active.find((ally2) => ally2 && ally2 !== target && !ally2.fainted);
        if (ally) {
