...
const Moves = {
  ...
  healingstones: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (pokemon.hasItem("heavydutyboots") || pokemon.hasAbility("overcoat") || pokemon.hasItem("dancingshoes") || pokemon.hasItem("mantisclaw"))
          return;
        let healAmounts = [0, 3];
    ...
    condition: {
      onSwap(target) {
        if (!target.fainted) {
          ...
          if (damage)
            this.add("-heal", target, target.getHealth, "[from] move: Life Dew", "[of] " + this.effectState.source);
      ...
      Object.assign(target.side.slotConditions[target.position]["futuremove"], {
        ...
        moveData: {
          ...
          onPrepareHit(target2, source2, move) {
            this.add("-anim", source2, "Sandsear Storm", target2);
    ...
    onTry(source, target) {
      ...
      Object.assign(target.side.slotConditions[target.position]["futuremove"], {
        ...
        moveData: {
          ...
          status: "tox",
          onPrepareHit(target2, source2, move) {
            this.add("-anim", source2, "Corrosive Gas", target2);
          },
          boosts: {
            def: -1,
            spd: -1
  ...
  toxicspikes: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        ...
        } else if (pokemon.hasType("Steel") || pokemon.hasItem("heavydutyboots") || pokemon.hasItem("dancingshoes") || pokemon.hasAbility("overcoat") || pokemon.hasItem("mantisclaw")) {
          return;
  ...
  stickyweb: {
    ...
    condition: {
      ...
      onEntryHazard(pokemon) {
        if (!pokemon.isGrounded() || pokemon.hasItem("heavydutyboots") || pokemon.hasItem("dancingshoes") || pokemon.hasAbility("overcoat") || pokemon.hasItem("mantisclaw"))
          return;
