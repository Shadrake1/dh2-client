...
const Abilities = {
    ...
    condition: {
      onSwap(target) {
        if (!target.fainted) {
          ...
          if (damage)
            this.add("-heal", target, target.getHealth, "[from] ability: Healer", "[of] " + this.effectState.source);
  ...
  sandspit: {
    ...
    condition: {
      ...
      onStart(pokemon, source) {
        ...
        this.effectState.boundDivisor = /*source.hasItem('bindingband') ? 8 :*/
        8;
      },
      onResidualOrder: 13,
      onResidual(pokemon) {
    ...
    onEnd(source) {
      ...
      for (const pokemon of this.getAllActive()) {
        ...
        if (pokemon.ignoringItem())
          continue;
        if (pokemon.hasItem("psychicseed") && this.field.isTerrain("psychicterrain") || pokemon.hasItem("electricseed") && this.field.isTerrain("electricterrain") || pokemon.hasItem("grassyseed") && this.field.isTerrain("grassyterrain") || pokemon.hasItem("mistyseed") && this.field.isTerrain("mistyterrain")) {
          for (const target of this.getAllActive()) {
            if (target.hasAbility("cloudnine") && target !== source) {
  ...
  runedrive: {
    ...
    condition: {
      ...
      onModifyAtk(atk, source, target, move) {
        if (this.effectState.bestStat !== "atk" || source.volatiles["cloudnine"])
          return;
      ...
      onModifyDef(def, target, source, move) {
        if (this.effectState.bestStat !== "def" || target.volatiles["cloudnine"])
          return;
      ...
      onModifySpA(relayVar, source, target, move) {
        if (this.effectState.bestStat !== "spa" || source.volatiles["cloudnine"])
          return;
      ...
      onModifySpD(relayVar, target, source, move) {
        if (this.effectState.bestStat !== "spd" || target.volatiles["cloudnine"])
          return;
        this.debug("Rune Drive spd boost");
        return this.chainModify([5325, 4096]);
      },
      onModifySpe(spe, pokemon) {
        if (this.effectState.bestStat !== "spe" || pokemon.volatiles["cloudnine"])
          return;
  ...
  sandveil: {
    // add sand clock effect here later
    name: "Sand Veil",
    onDamage(damage, target, source, effect) {
  ...
  snowcloak: {
    // add snow globe effect here later
    name: "Snow Cloak",
    onDamage(damage, target, source, effect) {
  ...
  outclass: {
    onSourceHit(target, source, move) {
      if (!move || !target || source.types[1] || source.volatiles["outclass"] || target.hasItem("terashard"))
        return;
      let targetType = target.types[0];
      if (target !== source && move.category !== "Status" && !source.hasType(targetType) && source.addType(targetType) && targetType !== "???") {
  ...
  colorchange: {
    ...
    onTryHit(target, source, move) {
      ...
      if (target.isActive && move.effectType === "Move" && target !== source && type !== "???" && !target.hasItem("terashard")) {
  ...
  */
  ...
  fairyringer: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      if (move.type !== "Fairy" || move.flags["pledgecombo"])
        return;
  ...
  mimicry: {
    shortDesc: "This Pok\xE9mon's type changes to match the Terrain. Type reverts when Terrain ends.",
    onStart(pokemon) {
    ...
    condition: {
      onStart(pokemon) {
        ...
        if (pokemon.getTypes().join() === newType || !pokemon.setType(newType))
          return;
  ...
  libero: {
    onPrepareHit(source, target, move) {
      if (this.effectState.libero || move.hasBounced || move.flags["futuremove"] || move.sourceEffect === "snatch")
        return;
      const type = move.type;
  ...
  protean: {
    onPrepareHit(source, target, move) {
      if (this.effectState.protean || move.hasBounced || move.flags["futuremove"] || move.sourceEffect === "snatch")
        return;
      const type = move.type;
      if (type && type !== "???" && !source.hasItem("terashard") && source.getTypes().join() !== type && source.setType(type)) {
