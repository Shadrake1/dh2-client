...
class Poll {
  ...
  generateResults(ended = false, option = 0) {
    ...
    if (option === 0 && !ended)
      output += "<div><small>(You can't vote after viewing results)</small></div>";
    output += "</div>";
    return output;
  }
  getQuestionMarkup() {
  ...
  poll: {
    ...
    new(target, room, user, connection, cmd, message) {
      ...
      if (target.length > 1024)
        return this.errorReply("Poll too long.");
      if (room.battle)
        return this.errorReply("Battles do not support polls.");
      const text = this.filter(target);
      if (target !== text)
        return this.errorReply("You are not allowed to use filtered words in polls.");
      ...
      if (!this.can("minigame", null, room))
        return false;
      if (supportHTML && !this.can("declare", null, room))
        return false;
      if (!this.canTalk())
        return;
      ...
      if (params.length < 3)
        return this.errorReply("Not enough arguments for /poll new.");
      if (supportHTML)
        params = params.map((parameter) => this.canHTML(parameter));
      if (params.some((parameter) => !parameter))
        return;
    ...
    vote(target, room, user) {
      ...
      if (isNaN(parsed))
        return this.errorReply("To vote, specify the number of the option.");
      if (!poll.options.has(parsed))
        return this.sendReply("Option not in poll.");
    ...
    timer(target, room, user) {
      ...
      if (target) {
        ...
        if (target === "clear") {
          if (!poll.timeout)
            return this.errorReply("There is no timer to clear.");
        ...
        if (isNaN(timeout) || timeout <= 0 || timeout > 2147483647)
          return this.errorReply("Invalid time given.");
        if (poll.timeout)
          clearTimeout(poll.timeout);
