...
class Hangman extends Rooms.RoomGame {
  ...
  guess(word, user) {
    if (user.id === this.creator)
      return user.sendTo(this.room, "You can't guess in your own hangman game.");
    const sanitized = word.replace(/[^A-Za-z ]/g, "");
    const normalized = toID(sanitized);
    if (normalized.length < 1)
      return user.sendTo(this.room, "Guess too short.");
    if (sanitized.length > 30)
      return user.sendTo(this.room, "Guess too long.");
    for (const guessid of this.guesses) {
      if (normalized === toID(guessid))
        return user.sendTo(this.room, "Your guess has already been guessed.");
    }
  ...
  generateWindow() {
    ...
    if (this.hint)
      output += Chat.html`<div>(Hint: ${this.hint})</div>`;
...
const commands = {
  hangman: {
    ...
    new(target, room, user, connection) {
      ...
      if (target !== text)
        return this.errorReply("You are not allowed to use filtered words in hangmans.");
      const params = text.split(",");
      if (!this.can("minigame", null, room))
        return false;
      if (room.hangmanDisabled)
        return this.errorReply("Hangman is disabled for this room.");
      if (!this.canTalk())
        return;
      if (room.game)
        return this.errorReply(`There is already a game of ${room.game.title} in progress in this room.`);
      if (!params)
        return this.errorReply("No word entered.");
      const word = params[0].replace(/[^A-Za-z '-]/g, "");
      if (word.replace(/ /g, "").length < 1)
        return this.errorReply("Enter a valid word");
      if (word.length > 30)
        return this.errorReply("Phrase must be less than 30 characters.");
      if (word.split(" ").some((w) => w.length > 20))
        return this.errorReply("Each word in the phrase must be less than 20 characters.");
      if (!/[a-zA-Z]/.test(word))
        return this.errorReply("Word must contain at least one letter.");
      let hint;
      if (params.length > 1) {
        hint = params.slice(1).join(",").trim();
        if (hint.length > 150)
          return this.errorReply("Hint too long.");
      }
    ...
    guess(target, room, user) {
      if (!target)
        return this.parse("/help guess");
      if (!room.game || room.game.gameid !== "hangman")
        return this.errorReply("There is no game of hangman running in this room.");
      if (!this.canTalk())
        return;
    ...
    display(target, room, user) {
      if (!room.game || room.game.title !== "Hangman")
        return this.errorReply("There is no game of hangman running in this room.");
  ...
  guess(target, room, user) {
    if (!room.game)
      return this.errorReply("There is no game running in this room.");
    if (!this.canTalk())
      return;
    if (!room.game.guess)
      return this.errorReply("You can't guess anything in this game.");
