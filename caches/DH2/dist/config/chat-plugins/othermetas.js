...
const commands = {
  ...
  mixandmega(target, room, user) {
    ...
    if (mixedTemplate.types[0] === deltas.type) {
    ...
    for (statName in template.baseStats) {
      mixedTemplate.baseStats[statName] = Dex.clampIntRange(
        mixedTemplate.baseStats[statName] + deltas.baseStats[statName],
        1,
        255
      );
    }
    ...
    let weighthit = 20;
    if (mixedTemplate.weighthg >= 2e3) {
      weighthit = 120;
    } else if (mixedTemplate.weighthg >= 1e3) {
      weighthit = 100;
    ...
    if (mixedTemplate.eggGroups)
      details["Egg Group(s)"] = mixedTemplate.eggGroups.join(", ");
  ...
  stone(target) {
    ...
    let baseTemplate = dex.getTemplate(stone.megaEvolves);
    let megaTemplate = dex.getTemplate(stone.megaStone);
    if (dex.gen >= 8 && ["redorb", "blueorb"].includes(stone.id))
      return this.parse("/help stone");
    if (stone.id === "redorb") {
      megaTemplate = dex.getTemplate("Groudon-Primal");
      baseTemplate = dex.getTemplate("Groudon");
  ...
  tiershift(target, room, user) {
    ...
    if (tier[0] === "(")
      tier = tier.slice(1, -1);
    if (!(tier in boosts))
      return this.sendReply(`|html|${Chat.getDataPokemonHTML(template)}`);
  ...
  scalemons(target, room, user) {
    ...
    const template = Dex.deepClone(Dex.getTemplate(target));
    if (!template.exists)
      return this.errorReply(`Error: Pokemon ${target} not found.`);
  ...
  natureswap(target, room, user) {
    ...
    if (!natureObj.exists)
      return this.errorReply(`Error: Nature ${nature} not found.`);
    const template = Dex.deepClone(Dex.getTemplate(pokemon));
    if (!template.exists)
      return this.errorReply(`Error: Pokemon ${pokemon} not found.`);
  ...
  crossevolve(target, user, room) {
    ...
    if (!template.exists)
      return this.errorReply(`Error: Pokemon '${pokes[0]}' not found.`);
    if (!crossTemplate.exists)
      return this.errorReply(`Error: Pokemon '${pokes[1]}' not found.`);
    if (!template.evos.length)
      return this.errorReply(`Error: ${template.species} does not evolve.`);
    if (!crossTemplate.prevo)
      return this.errorReply(`Error: ${crossTemplate.species} does not have a prevolution.`);
    let setStage = 1;
    let crossStage = 1;
    if (template.prevo) {
      setStage++;
    ...
    if (template.types[1])
      mixedTemplate.types.push(template.types[1]);
    if (crossTemplate.types[0] !== prevo.types[0])
      mixedTemplate.types[0] = crossTemplate.types[0];
    if (crossTemplate.types[1] !== prevo.types[1]) {
      mixedTemplate.types[1] = crossTemplate.types[1] || crossTemplate.types[0];
    }
    if (mixedTemplate.types[0] === mixedTemplate.types[1])
      mixedTemplate.types = [mixedTemplate.types[0]];
