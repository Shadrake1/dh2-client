...
const MafiaData = require("../../server/chat-plugins/mafia-data.js");
let logs = { leaderboard: {}, mvps: {}, hosts: {}, plays: {}, leavers: {} };
let hostBans = /* @__PURE__ */ Object.create(null);
const hostQueue = [];
const IDEA_TIMER = 90 * 1e3;
function readFile(path) {
  try {
    const json = (0, import_fs.FS)(path).readIfExistsSync();
    if (!json) {
      writeFile(path, {});
      return false;
    }
...
function writeFile(path, data) {
  (0, import_fs.FS)(path).writeUpdate(() => JSON.stringify(data));
}
logs = readFile(LOGS_FILE);
if (!logs)
  logs = { leaderboard: {}, mvps: {}, hosts: {}, plays: {}, leavers: {} };
...
if (!hostBans)
  hostBans = /* @__PURE__ */ Object.create(null);
...
class MafiaTracker extends Rooms.RoomGame {
  constructor(room, host) {
    ...
    this.dead = /* @__PURE__ */ Object.create(null);
    this.subs = [];
    this.autoSub = true;
    this.requestedSub = [];
    ...
    this.played = [];
    this.hammerCount = 0;
    this.lynches = /* @__PURE__ */ Object.create(null);
    this.lynchModifiers = /* @__PURE__ */ Object.create(null);
  ...
  join(user) {
    if (this.phase !== "signups")
      return user.sendTo(this.room, `|error|The game of ${this.title} has already started.`);
    const canJoin = this.canJoin(user, true);
    if (canJoin)
      return user.sendTo(this.room, `|error|${canJoin}`);
    if (this.playerCount >= this.playerCap)
      return user.sendTo(this.room, `|error|The game of ${this.title} is full.`);
  ...
  setRoles(user, roleString, force = false, reset = false) {
    ...
    if (roles.length === 1) {
      ...
      if (themeName in MafiaData.themes) {
        ...
        if (typeof theme === "object") {
          ...
          const themeRoles = theme[this.playerCount].slice();
    ...
    for (const roleName of roles) {
      ...
      if (roleId in cache) {
        newRoles.push(Object.assign(/* @__PURE__ */ Object.create(null), cache[roleId]));
      } else {
        const role = MafiaTracker.parseRole(roleName);
        if (role.problems.length)
          problems = problems.concat(role.problems);
        if (alignments.indexOf(role.role.alignment) === -1)
  ...
  static parseRole(roleString) {
    ...
    for (let key in MafiaData.roles) {
      if (key.includes("_")) {
        ...
        if (roleKey.includes(key)) {
          ...
          if (typeof MafiaData.roles[key] === "string")
            key = MafiaData.roles[key];
          if (!role.image && MafiaData.roles[key].image)
            role.image = MafiaData.roles[key].image;
          ...
          if (MafiaData.roles[key].memo)
            role.memo = role.memo.concat(MafiaData.roles[key].memo);
    ...
    for (let key in MafiaData.modifiers) {
      if (key.includes("_")) {
        const roleKey = target.slice().map(toID).join("_");
        if (roleKey.includes(key)) {
          if (typeof MafiaData.modifiers[key] === "string")
            key = MafiaData.modifiers[key];
          if (!role.image && MafiaData.modifiers[key].image)
            role.image = MafiaData.modifiers[key].image;
          if (MafiaData.modifiers[key].memo)
            role.memo = role.memo.concat(MafiaData.modifiers[key].memo);
    ...
    for (let [j, targetId] of target.entries()) {
      ...
      if (MafiaData.alignments[id]) {
        if (typeof MafiaData.alignments[id] === "string")
          id = MafiaData.alignments[id];
        ...
        if (!role.image && MafiaData.alignments[id].image)
          role.image = MafiaData.alignments[id].image;
        target.splice(j, 1);
        j--;
      }
    }
  ...
  start(user, night = false) {
    if (!user)
      return;
    if (this.phase !== "locked" && this.phase !== "IDEAlocked") {
      if (this.phase === "signups")
        return user.sendTo(this.room, `You need to close the signups first.`);
    ...
    if (this.playerCount < 2)
      return user.sendTo(this.room, `You need at least 2 players to start.`);
    if (this.phase === "IDEAlocked") {
      for (const p in this.playerTable) {
        if (!this.playerTable[p].role)
          return user.sendTo(this.room, `|error|Not all players have a role.`);
      }
    } else {
      if (!Object.keys(this.roles).length)
        return user.sendTo(this.room, `You need to set the roles before starting.`);
...
|notify|Your role is ${role.safeName}. For more details of your role, check your Role PM.`);
    ...
    this.dead = {};
    this.played = [this.hostid, ...this.cohosts, ...Object.keys(this.playerTable)];
    this.sendDeclare(`The roles have been distributed.`);
    this.updatePlayers();
  }
  getPartners(alignment, player) {
    if (!player || !player.role || ["town", "solo", "traitor"].includes(player.role.alignment))
      return "";
    const partners = [];
    for (const p in this.playerTable) {
      if (p === player.id)
      ...
      if (role && role.alignment === player.role.alignment)
        partners.push(this.playerTable[p].name);
    }
...
|notify|It's night in your game of Mafia!`);
  ...
  lynch(userid, target) {
    if (this.phase !== "day")
      return this.sendUser(userid, `|error|You can only lynch during the day.`);
    let player = this.playerTable[userid];
    if (!player && this.dead[userid] && this.dead[userid].restless)
      player = this.dead[userid];
    if (!player)
      return;
    if (!(target in this.playerTable) && target !== "nolynch")
      return this.sendUser(userid, `|error|${target} is not a valid player.`);
    if (!this.enableNL && target === "nolynch")
      return this.sendUser(userid, `|error|No Lynch is not allowed.`);
    if (target === player.id && !this.selfEnabled)
      return this.sendUser(userid, `|error|Self lynching is not allowed.`);
    if (target === player.id && this.hammerCount - 1 > (this.lynches[target] ? this.lynches[target].count : 0) && this.selfEnabled === "hammer") {
      return this.sendUser(userid, `|error|You may only lynch yourself when you placing the hammer vote.`);
    }
    if (player.lastLynch + 2e3 >= Date.now())
      return this.sendUser(userid, `|error|You must wait another ${Chat.toDurationString(player.lastLynch + 2e3 - Date.now()) || "1 second"} before you can change your lynch.`);
    const previousLynch = player.lynching;
    if (previousLynch)
      this.unlynch(userid, true);
    ...
    if (this.getHammerValue(target) <= lynch.trueCount) {
      ...
      this.sendRoom(`|raw|<div class="infobox">${this.lynchBox()}</div>`);
      if (target !== "nolynch")
        this.eliminate(this.playerTable[target], "kill");
      this.night(true);
      return;
    }
    this.hasPlurality = null;
  ...
  unlynch(userid, force = false) {
    ...
    if (!player && this.dead[userid] && this.dead[userid].restless)
      player = this.dead[userid];
    if (!player || !player.lynching)
      return this.sendUser(userid, `|error|You are not lynching anyone.`);
    if (player.lastLynch + 2e3 >= Date.now() && !force)
      return this.sendUser(userid, `|error|You must wait another ${Chat.toDurationString(player.lastLynch + 2e3 - Date.now()) || "1 second"} before you can change your lynch.`);
    ...
    const targetUser = Users.get(userid);
    if (!force)
      this.sendTimestamp(player.lynching === "nolynch" ? `${targetUser ? targetUser.name : userid} is no longer abstaining from lynching.` : `${targetUser ? targetUser.name : userid} has unlynched ${this.playerTable[player.lynching].name}.`);
    player.lynching = "";
    player.lastLynch = Date.now();
    this.hasPlurality = null;
    this.updatePlayersLynches();
  ...
  lynchBoxFor(userid) {
    ...
    for (const key of Object.keys(this.playerTable).concat(this.enableNL ? ["nolynch"] : [])) {
      ...
      if (isPlayer || isSpirit) {
        ...
        if (lynch && lynch.count !== lynch.trueCount)
          buf += `(${lynch.trueCount})`;
  ...
  applyLynchModifier(user, target, mod) {
    ...
    if (mod === oldMod || (isNaN(mod) || mod === 1) && oldMod === void 0) {
      if (isNaN(mod) || mod === 1)
        return this.sendUser(user, `|error|${target} already has no lynch modifier.`);
  ...
  clearLynchModifiers(user) {
    for (const player of [...Object.keys(this.playerTable), ...Object.keys(this.dead)]) {
      if (this.lynchModifiers[player])
        this.applyLynchModifier(user, player, 1);
    }
  }
  clearHammerModifiers(user) {
    for (const player of ["nolynch", ...Object.keys(this.playerTable)]) {
      if (this.hammerModifiers[player])
  ...
  shiftHammer(count) {
    ...
    for (const lynch in this.lynches) {
      if (this.lynches[lynch].trueCount >= this.getHammerValue(lynch)) {
  ...
  getPlurality() {
    ...
    if (!Object.keys(this.lynches).length)
      return null;
    let max = 0;
    let topLynches = [];
  ...
  revive(user, toRevive, force = false) {
    ...
    if (toRevive in this.dead) {
      ...
      if (deadPlayer.treestump)
        deadPlayer.treestump = false;
      if (deadPlayer.restless)
        deadPlayer.restless = false;
      ...
      if (this.subs.includes(targetUser.id))
        this.subs.splice(this.subs.indexOf(targetUser.id), 1);
  ...
  sub(player, replacement) {
    ...
    if (this.lynches[oldPlayer.id]) {
      ...
      for (const p in this.playerTable) {
        if (this.playerTable[p].lynching === oldPlayer.id)
          this.playerTable[p].lynching = newPlayer.id;
      }
      for (const p in this.dead) {
        if (this.dead[p].restless && this.dead[p].lynching === oldPlayer.id)
          this.dead[p].lynching = newPlayer.id;
      }
    }
...
|notify|You have been substituted in the mafia game for ${oldPlayer.safeName}.`);
    ...
    if (this.room.roomid === "mafia" && this.started) {
      ...
      if (!logs.leavers[month])
        logs.leavers[month] = {};
      if (!logs.leavers[month][player])
        logs.leavers[month][player] = 0;
  ...
  nextSub(userid = null) {
    ...
    if (!sub || !sub.connected || !sub.named || !this.room.users[sub.id])
      return;
  ...
  ideaInit(user, moduleName) {
    ...
    if (typeof this.IDEA.data === "string")
      this.IDEA.data = MafiaData.IDEAs[this.IDEA.data];
    if (!this.IDEA.data)
      return user.sendTo(this.room, `|error|${moduleName} is not a valid IDEA.`);
    if (typeof this.IDEA.data !== "object")
      return this.sendRoom(`Invalid alias for IDEA ${moduleName}. Please report this to a mod.`);
    return this.ideaDistributeRoles(user);
  }
  ideaDistributeRoles(user) {
    if (!this.IDEA.data)
      return user.sendTo(this.room, `|error|No IDEA module loaded`);
    if (this.phase !== "locked" && this.phase !== "IDEAlocked")
...
|notify|Pick your role in the IDEA module.`);
  ...
  ideaPick(user, selection) {
    ...
    if (selection.length !== 2)
      return user.sendTo(this.room, `|error|Invalid selection.`);
    if (selection[1]) {
  ...
  ideaFinalizePicks() {
    ...
    for (const p in this.playerTable) {
      ...
      if (this.IDEA.data.picks.length === 1) {
        ...
        if (parsedRole.problems.length && !this.IDEA.data.untrusted)
    ...
    for (const p of Object.keys(this.playerTable).sort()) {
      ...
      if (!IDEA)
        return this.sendRoom(`No IDEA data for player ${p} when finalising IDEAs. Please report this to a mod.`);
  ...
  sendPlayerList() {
    this.room.add(`|c:|${Math.floor(Date.now() / 1e3)}|~|**Players (${this.playerCount})**: ${Object.keys(this.playerTable).map((p) => this.playerTable[p].name).join(", ")}`).update();
  }
  updatePlayers() {
    for (const p in this.playerTable) {
    ...
    for (const p in this.dead) {
      if (this.dead[p].restless || this.dead[p].treestump)
        this.dead[p].updateHtmlRoom();
    }
    this.updateHost();
  }
  ...
  canJoin(user, self = false, force = false) {
    ...
    if (!this.room.users[user.id])
      return `${targetString} not in the room.`;
    if (this.playerTable[user.id])
      return `${targetString} already in the game.`;
    if (this.hostid === user.id)
      return `${targetString} the host.`;
    if (this.cohosts.includes(user.id))
      return `${targetString} a cohost.`;
    if (!force) {
  ...
  sendUser(user, message) {
    ...
    if (!userObject || !userObject.connected)
      return;
  ...
  setSelfLynch(user, setting) {
    ...
    if (from === setting)
      return user.sendTo(this.room, `|error|Selflynching is already ${setting ? `set to Self${setting === "hammer" ? "hammering" : "lynching"}` : "disabled"}.`);
    if (from) {
  ...
  setNoLynch(user, setting) {
    ...
    if (!setting)
      this.clearLynches("nolynch");
    this.updatePlayers();
  }
  clearLynches(target = "") {
    if (target)
      delete this.lynches[target];
    if (!target)
      this.lynches = /* @__PURE__ */ Object.create(null);
    for (const player of Object.values(this.playerTable)) {
      if (this.forceLynch) {
    ...
    for (const player of Object.values(this.dead)) {
      if (player.restless && (!target || player.lynching === target))
        player.lynching = "";
    }
    this.hasPlurality = null;
  }
  onChatMessage(message, user) {
    ...
    if (user.isStaff || this.room.auth && this.room.auth[user.id] && this.room.auth[user.id] !== "+" || this.hostid === user.id || this.cohosts.includes(user.id) || !this.started) {
      return false;
    }
    if (!this.playerTable[user.id] && (!this.dead[user.id] || !this.dead[user.id].treestump))
      return `You cannot talk while a game of ${this.title} is going on.`;
    if (this.phase === "night")
      return `You cannot talk at night.`;
    return false;
  }
  onConnect(user) {
  ...
  onLeave(user, oldUserid) {
    ...
    if (this.subs.includes(userid))
      this.subs.splice(this.subs.indexOf(userid), 1);
  }
...
const pages = {
  mafia(query, user) {
    ...
    if (isHost) {
      ...
      for (const p in game.playerTable) {
        ...
        buf += game.lynchModifiers[p] !== void 0 ? `(lynches worth ${game.getLynchValue(p)})` : "";
      ...
      for (const d in game.dead) {
        ...
        if (dead.restless)
          buf += ` (is a Restless Spirit)`;
        if (game.lynchModifiers[d] !== void 0)
          buf += ` (lynches worth ${game.getLynchValue(d)})`;
  ...
  mafialadder(query, user) {
    if (!user.named)
      return Rooms.RETRY_AFTER_LOGIN;
    if (!query.length || !Rooms.get("mafia"))
      return this.close();
    const headers = {
    ...
    if (["hosts", "plays", "leavers"].includes(ladder.section) && !this.can("mute", null, mafiaRoom))
      return;
  ...
  mafia: {
    ...
    host(target, room, user, connection, cmd) {
      if (room.mafiaDisabled)
        return this.errorReply(`Mafia is disabled for this room.`);
      if (!this.canTalk())
        return;
      if (!room || room.type !== "chat")
        return this.errorReply(`This command is only meant to be used in chat rooms.`);
      if (room.game)
      ...
      if (!user.can("broadcast", null, room))
        return this.errorReply(`/mafia ${cmd} - Access denied.`);
      let nextHost = false;
      if (room.roomid === "mafia") {
        if (cmd === "nexthost") {
          nextHost = true;
          if (!hostQueue.length)
            return this.errorReply(`Nobody is on the host queue.`);
          const skipped = [];
          do {
      ...
      if (!nextHost && this.targetUser.id !== user.id && !this.can("mute", null, room))
        return false;
    ...
    queue(target, room, user) {
      if (room.mafiaDisabled)
        return this.errorReply(`Mafia is disabled for this room.`);
      if (room.roomid !== "mafia")
        return this.errorReply(`This command can only be used in the Mafia room.`);
      ...
      if (["forceadd", "add", "remove", "del", "delete"].includes(args[0])) {
        ...
        if (["forceadd", "add"].includes(args[0]) && !this.can(permission, null, room))
          return;
        if (["remove", "del", "delete"].includes(args[0]) && user.id !== args[1] && !this.can("mute", null, room))
          return;
      } else {
        if (!this.runBroadcast())
          return false;
      ...
      switch (args[0]) {
        ...
        case "add":
          ...
          if (hostQueue.includes(args[1]))
            return this.errorReply(`User ${args[1]} is already on the host queue.`);
          if (isHostBanned(args[1]))
            return this.errorReply(`User ${args[1]} is banned from hosting games.`);
        ...
        case "remove":
          ...
          if (index === -1)
            return this.errorReply(`User ${args[1]} is not on the host queue.`);
    ...
    join(target, room, user) {
      ...
      if (!this.canTalk(null, targetRoom))
        return;
      game.join(user);
    },
    ...
    playercap(target, room, user) {
      if (!room || !room.game || room.game.gameid !== "mafia")
        return this.errorReply(`There is no game of mafia running in this room.`);
      const game = room.game;
      if (game.hostid !== user.id && !game.cohosts.includes(user.id) && !this.can("mute", null, room))
        return;
      if (game.phase !== "signups")
        return this.errorReply(`Signups are already closed.`);
      if (toID(target) === "none")
        target = "20";
      const num = parseInt(target);
      if (isNaN(num) || num > 20 || num < 2)
      ...
      if (num === game.playerCap)
        return this.errorReply(`Player cap is already set at ${game.playerCap}.`);
      game.playerCap = num;
    ...
    close(target, room, user) {
      ...
      if (game.hostid !== user.id && !game.cohosts.includes(user.id) && !this.can("mute", null, room))
        return;
      if (game.phase !== "signups")
        return user.sendTo(targetRoom, `|error|Signups are already closed.`);
      if (game.playerCount < 2)
        return user.sendTo(targetRoom, `|error|You need at least 2 players to start.`);
      game.phase = "locked";
    ...
    closedsetup(target, room, user) {
      ...
      if (game.started)
        return user.sendTo(targetRoom, `|error|You can't ${action === "on" ? "enable" : "disable"} closed setup because the game has already started.`);
    ...
    reveal(target, room, user) {
      ...
      if (action === "off" && game.noReveal || action === "on" && !game.noReveal)
        return user.sendTo(targetRoom, `|error|Revealing of roles is already ${game.noReveal ? "disabled" : "enabled"}.`);
    ...
    setroles(target, room, user, connection, cmd) {
      ...
      if (reset) {
        if (game.phase !== "day" && game.phase !== "night")
          return this.errorReply(`The game has not started yet.`);
      } else {
        if (game.phase !== "locked" && game.phase !== "IDEAlocked")
    ...
    idea(target, room, user) {
      if (!room.game || room.game.gameid !== "mafia")
        return this.errorReply(`There is no game of mafia running in this room.`);
      const game = room.game;
      if (!this.can("broadcast", null, room))
        return;
      ...
      if (game.started)
        return this.errorReply(`You cannot start an IDEA after the game has started.`);
      if (game.phase !== "locked" && game.phase !== "IDEAlocked")
    ...
    customidea(target, room, user) {
      ...
      if (!choices || choices <= picks.length)
        return this.errorReply(`You need to have more choices than picks.`);
    ...
    ideadiscards(target, room, user) {
      ...
      const game = room.game;
      if (!game.IDEA.data)
        return this.errorReply(`There is no IDEA module in the mafia game.`);
      if (target) {
        if (game.hostid !== user.id && !game.cohosts.includes(user.id) && !this.can("mute", null, room))
          return;
        if (this.meansNo(target)) {
          if (game.IDEA.discardsHidden)
            return this.errorReply(`IDEA discards are already hidden.`);
          game.IDEA.discardsHidden = true;
        } else if (this.meansYes(target)) {
          if (!game.IDEA.discardsHidden)
            return this.errorReply(`IDEA discards are already visible.`);
      ...
      if (game.IDEA.discardsHidden)
        return this.errorReply(`Discards are not visible.`);
      if (!game.IDEA.discardsHTML)
        return this.errorReply(`The IDEA module does not have finalised discards yet.`);
      if (!this.runBroadcast())
        return;
    ...
    lynch(target, room, user) {
      ...
      if (!this.canTalk(null, targetRoom))
        return;
      if (!(user.id in game.playerTable) && (!(user.id in game.dead) || !game.dead[user.id].restless)) {
    ...
    kill(target, room, user, connection, cmd) {
      ...
      if (!player)
        return user.sendTo(targetRoom, `|error|"${args.join(",")}" is not a living player.`);
      if (game.phase === "IDEApicking")
        return this.errorReply(`You cannot add or remove players while IDEA roles are being picked.`);
    ...
    deadline(target, room, user) {
      ...
      const game = room.game;
      target = toID(target);
      if (target && game.hostid !== user.id && !game.cohosts.includes(user.id) && !this.can("mute", null, room))
        return;
      if (target === "off") {
        ...
        if (num < 1 || num > 20)
          return this.errorReply(`The deadline must be between 1 and 20 minutes.`);
    ...
    silence(target, room, user, connection, cmd) {
      ...
      if (!targetPlayer)
        return this.errorReply(`${target} is not in the game of mafia.`);
    ...
    forcelynch(target, room, user) {
      ...
      if (this.meansYes(target)) {
        if (game.forceLynch)
          return this.errorReply(`Forcelynching is already enabled.`);
        ...
        if (!game.forceLynch)
          return this.errorReply(`Forcelynching is already disabled.`);
    ...
    lynches(target, room, user) {
      ...
      const game = room.game;
      if (!game.started)
        return this.errorReply(`The game of mafia has not started yet.`);
      if (game.hostid === user.id || game.cohosts.includes(user.id)) {
        this.broadcastMessage = this.message.toLowerCase().replace(/[^a-z0-9\s!,]/g, "");
      }
      if (!this.runBroadcast())
    ...
    rolelist(target, room, user, connection, cmd) {
      ...
      const game = room.game;
      if (game.closedSetup)
        return this.errorReply(`You cannot show roles in a closed setup.`);
      if (!this.runBroadcast())
        return false;
      if (game.IDEA.data) {
    ...
    playerroles(target, room, user) {
      ...
      if (!game.started)
        return this.errorReply(`The game has not started.`);
...
|selectorhtml|#mafia-lynches|` + lynches);
      ...
      switch (action) {
        case "in":
          if (user.id in game.playerTable) {
            ...
            if (game.subs.includes(user.id))
              return user.sendTo(targetRoom, `|error|You are already on the sub list.`);
            if (game.played.includes(user.id))
              return user.sendTo(targetRoom, `|error|You cannot sub back into the game.`);
            const canJoin2 = game.canJoin(user, true);
            if (canJoin2)
              return user.sendTo(targetRoom, `|error|${canJoin2}`);
        ...
        case "out":
          if (user.id in game.playerTable) {
            ...
            if (!game.subs.includes(user.id))
              return user.sendTo(targetRoom, `|error|You are not on the sub list.`);
        ...
        case "next":
          ...
          if (!(toID(toSub) in game.playerTable))
            return user.sendTo(targetRoom, `|error|${toSub} is not in the game.`);
          if (!game.subs.length) {
            if (game.hostRequestedSub.indexOf(toID(toSub)) !== -1)
        ...
        case "remove":
          ...
          if (toRemoveIndex === -1)
            return user.sendTo(room, `|error|${toRemove} is not on the sub list.`);
        ...
        default:
          ...
          if (!(toSubOut in game.playerTable))
            return user.sendTo(targetRoom, `|error|${toSubOut} is not in the game.`);
          const targetUser = Users.get(toSubIn);
          if (!targetUser)
            return user.sendTo(targetRoom, `|error|The user "${toSubIn}" was not found.`);
          const canJoin = game.canJoin(targetUser, false, cmd === "forcesub");
          if (canJoin)
            return user.sendTo(targetRoom, `|error|${canJoin}`);
    ...
    autosub(target, room, user) {
      ...
      if (this.meansYes(toID(args.join("")))) {
        if (game.autoSub)
          return user.sendTo(targetRoom, `|error|Automatic subbing of players is already enabled.`);
        game.autoSub = true;
        user.sendTo(targetRoom, `Automatic subbing of players has been enabled.`);
        game.nextSub();
      } else if (this.meansNo(toID(args.join("")))) {
        if (!game.autoSub)
          return user.sendTo(targetRoom, `|error|Automatic subbing of players is already disabled.`);
        game.autoSub = false;
    ...
    subhost(target, room, user, connection, cmd) {
      ...
      if (!room.users[targetUser.id])
        return this.errorReply(`${targetUser.name} is not in this room, and cannot be hosted.`);
      if (game.hostid === targetUser.id)
        return this.errorReply(`${targetUser.name} is already the host.`);
      if (game.cohosts.includes(targetUser.id))
        return this.errorReply(`${targetUser.name} is already a cohost.`);
      if (targetUser.id in game.playerTable)
        return this.errorReply(`The host cannot be ingame.`);
      if (targetUser.id in game.dead) {
        ...
        if (game.dead[targetUser.id].lynching)
          game.unlynch(targetUser.id);
...
|deinit`);
    ...
    "!end": true,
      ...
      if (!targetRoom.game || targetRoom.game.gameid !== "mafia")
        return user.sendTo(targetRoom, `|error|There is no game of mafia running in this room.`);
      const game = targetRoom.game;
      if (game.hostid !== user.id && !game.cohosts.includes(user.id) && !this.can("broadcast", null, room))
        return;
      game.end();
    ...
    data(target, room, user, connection, cmd) {
      if (room && room.mafiaDisabled)
        return this.errorReply(`Mafia is disabled for this room.`);
      if (cmd === "role" && !target && room) {
        const game2 = room.game;
        if (!game2 || game2.roomid !== "mafia")
        ...
        if (!(user.id in game2.playerTable))
          return this.errorReply(`You are not in the game of ${game2.title}.`);
        const role = game2.playerTable[user.id].role;
        if (!role)
          return this.errorReply(`You do not have a role yet.`);
        return this.sendReplyBox(`Your role is: ${role.safeName}`);
      }
      const game = room && room.game && room.game.gameid === "mafia" ? room.game : null;
      ...
      let dataType = cmd;
      if (cmd in types) {
        const type = types[cmd];
        const data = MafiaData[type];
        if (!data)
          return this.errorReply(`"${type}" is not a valid search area.`);
        if (!data[id])
          return this.errorReply(`"${target} is not a valid ${cmd}."`);
        result = data[id];
        ...
        if (!result)
          return this.errorReply(`"${target}" is not a valid mafia alignment, role, modifier, or theme.`);
      }
      ...
      if (dataType === "theme") {
        ...
        for (const i in result) {
          ...
          const count = {};
          const roles = [];
          for (const role of result[i].split(",").map((x) => x.trim())) {
    ...
    win(target, room, user, connection, cmd) {
      ...
      if (room.roomid !== "mafia")
        return this.errorReply(`This command can only be used in the Mafia room.`);
      if (cmd === "winfaction" && (!room.game || room.game.gameid !== "mafia"))
        return this.errorReply(`There is no game of mafia running in the room`);
      if (!this.can("mute", null, room))
        return;
      const args = target.split(",");
    ...
    hostban(target, room, user, connection, cmd) {
      ...
      if (isUnban) {
        ...
        if (duration > 7)
          return this.errorReply(`Bans cannot be longer than 7 days.`);
        hostBans[targetUserID] = Date.now() + 1e3 * 60 * 60 * 24 * duration;
    ...
    disable(target, room, user) {
      if (!room || !this.can("gamemanagement", null, room))
        return;
      if (room.mafiaDisabled) {
