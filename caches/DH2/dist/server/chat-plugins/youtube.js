...
const commands = {
  ...
  youtube: {
    async addchannel(target, room, user) {
      ...
      if (!id)
        return this.errorReply("Specify a channel ID.");
    ...
    removechannel(target, room, user) {
      ...
      if (!id)
        return this.errorReply(`Channel with ID or name ${target} not found.`);
    ...
    async channel(target, room, user) {
      ...
      if (!channel)
        return this.errorReply(`No channels with ID or name ${target} found.`);
    ...
    update(target, room, user) {
      ...
      if (!id)
        return this.errorReply(`Channel ${channel} is not in the database.`);
    ...
    repeat(target, room, user) {
      ...
      if (!target) {
        if (!YouTube.interval)
          return this.errorReply(`The YouTube plugin is not currently running an interval.`);
        return this.sendReply(`Interval is currently set to ${Chat.toDurationString(YouTube.intervalTime * 60 * 1e3)}.`);
      }
      if (this.meansNo(target)) {
        if (!YouTube.interval)
          return this.errorReply(`The interval is not currently running`);
        clearInterval(YouTube.interval);
      ...
      if (isNaN(parseInt(target)))
        return this.errorReply(`Specify a number (in minutes) for the interval.`);
    ...
    decategorize(target, room, user) {
      ...
      if (!category)
        return this.errorReply(`That channel does not have a category.`);
  ...
  groupwatch: {
    async create(target, room, user) {
      ...
      if (!url || !title)
        return this.errorReply(`You must specify a video to watch and a title for the group watch.`);
...
const pages = {
  ...
  groupwatch(query, user, connection) {
    ...
    if (!watch)
      return this.errorReply(`Groupwatch ${roomid}-${num} not found.`);
