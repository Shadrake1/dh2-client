...
var random_battles_exports = {};
__export(random_battles_exports, {
  commands: () => commands
});
module.exports = __toCommonJS(random_battles_exports);
...
function getRBYMoves(species) {
  ...
  if (!species.randomBattleMoves && !species.comboMoves && !species.exclusiveMoves && !species.essentialMove) {
    return false;
  }
  return buf;
}
function getLetsGoMoves(species) {
  ...
  if (!isLetsGoLegal)
    return false;
  if (!species.randomBattleMoves?.length)
    return false;
...
function battleFactorySets(species, tier, gen = "gen8", isBSS = false) {
  ...
  const statsFile = JSON.parse(
    (0, import_lib.FS)(`data${gen === "gen8" ? "/" : `/mods/${gen}`}/${isBSS ? `bss-` : ``}factory-sets.json`).readIfExistsSync() || "{}"
  );
...
function CAP1v1Sets(species) {
  species = Dex.species.get(species);
  const statsFile = JSON.parse(
    (0, import_lib.FS)(`data/cap-1v1-sets.json`).readIfExistsSync() || "{}"
  );
  if (!Object.keys(statsFile).length)
    return null;
...
function generateSSBMoveInfo(sigMove, dex) {
  ...
  if (sigMove.shortDesc || sigMove.desc) {
    ...
    buf += `<font size="1">${Object.entries(details).map(([detail, value]) => value === "" ? detail : `<font color="#686868">${detail}:</font> ${value}`).join("&nbsp;|&ThickSpace;")}</font>`;
...
const commands = {
  ...
  randombattles(target, room, user) {
    ...
    if (dex.gen === 1) {
      ...
      for (const pokemon of setsToCheck) {
        if (!pokemon.randomBattleMoves || pokemon.isNonstandard === "Future")
          continue;
        const randomMoves = pokemon.randomBattleMoves.slice();
        const m = randomMoves.sort().map(formatMove);
        movesets.push(
          `<details${!movesets.length ? " open" : ""}><summary><span style="color:#999999;">Moves for ${pokemon.name} in ${formatName}:<span style="color:#999999;"></summary>${m.join(`, `)}</details>`
        );
      }
    }
    if (!movesets.length) {
  ...
  randombattleshelp: [
    `/randombattles OR /randbats [pokemon], [gen] - Displays a Pok\xE9mon's Random Battle Moves. Defaults to Gen 8. If used in a battle, defaults to the gen of that battle.`
  ],
  ...
  randomdoublesbattle(target, room, user) {
    ...
    const movesets = [];
    for (const pokemon of setsToCheck) {
      if (!pokemon.randomDoubleBattleMoves)
        continue;
      const moves = [...pokemon.randomDoubleBattleMoves];
