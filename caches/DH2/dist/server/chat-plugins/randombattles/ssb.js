...
function generateSSBSet(set, dex, baseDex) {
  ...
  if (set.shiny)
    buf += `<li>Shiny: ${typeof set.shiny === "number" ? `1 in ${set.shiny} chance` : `Yes`}</li>`;
  if (set.evs) {
    const evs = [];
    let ev;
    for (ev in set.evs) {
      if (set.evs[ev] === 0)
        continue;
      evs.push(`${set.evs[ev]} ${import__.STAT_NAMES[ev]}`);
    }
    buf += `<li>EVs: ${evs.join(" / ")}</li>`;
  }
  if (set.nature) {
    buf += `<li>${Array.isArray(set.nature) ? set.nature.map((x) => (0, import__.formatNature)(x)).join(" / ") : (0, import__.formatNature)(set.nature)} Nature</li>`;
  }
  if (set.ivs) {
    const ivs = [];
    let iv;
    for (iv in set.ivs) {
      if (set.ivs[iv] === 31)
        continue;
      ivs.push(`${set.ivs[iv]} ${import__.STAT_NAMES[iv]}`);
    }
...
function generateSSBInnateInfo(name, dex, baseDex) {
  ...
  if (effect.exists && effect.innateName && (effect.desc || effect.shortDesc)) {
    ...
    if (effect.exists && effect.innateName) {
      baseAbility.name = effect.innateName;
      if (effect.desc)
        baseAbility.desc = effect.desc;
      if (effect.shortDesc)
...
function SSBSets(target) {
  ...
  for (const setName of sets) {
    ...
    if (!set.skip) {
      buf += import_lib.Utils.html`<h1><psicon pokemon="${mutatedSpecies.id}">${setName}</h1>`;
    } else {
