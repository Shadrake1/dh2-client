    ...
    for (const parameter of parameters) {
      ...
      if (target.endsWith(" asc") || target.endsWith(" desc")) {
        ...
        if (!allStats.includes(stat2))
          return { error: `'${escapeHTML(target)}' did not contain a valid stat.` };
    ...
    for (const mon in dex) {
      ...
      if (alts.tiers && Object.keys(alts.tiers).length) {
        ...
        if (Object.values(alts.tiers).includes(false) && alts.tiers[tier] !== false)
          continue;
        let format2 = Dex.formats.get("gen" + mod.gen + "lc");
        if (!format2.exists)
          format2 = Dex.formats.get("gen9lc");
        if (alts.tiers.LC && !dex[mon].prevo && dex[mon].nfe && !Dex.formats.getRuleTable(format2).isBannedSpecies(dex[mon])) {
      ...
      const validator = import_team_validator.TeamValidator.get(`${formatStr}${additionalRules.length ? `@@@${additionalRules.join(",")}` : ""}`);
...
function runMovesearch(target, cmd2, canAll, message, isTest) {
    ...
    for (const parameter of parameters) {
      ...
      if (target.endsWith(" asc") || target.endsWith(" desc")) {
        ...
        if (!allProperties.includes(prop))
          return { error: `'${escapeHTML(target)}' did not contain a valid property.` };
      ...
      if (target.substr(0, 7) === "boosts " || target.substr(0, 7) === "lowers ") {
        ...
        if (!allBoosts.includes(target))
          return { error: `'${escapeHTML(target)}' is not a recognized stat.` };
        if (isBoost) {
  ...
  for (const alts of searches) {
    ...
    for (const moveid in dex) {
      ...
      for (const searchStatus in alts.status) {
        let canStatus = !!(move.status === searchStatus || move.secondaries && move.secondaries.some((entry) => entry.status === searchStatus));
        if (searchStatus === "slp") {
      ...
      for (const searchStatus in alts.volatileStatus) {
        const canStatus = !!(move.secondary && move.secondary.volatileStatus === searchStatus || move.secondaries && move.secondaries.some((entry) => entry.volatileStatus === searchStatus) || move.volatileStatus === searchStatus);
  ...
  if (results.length > 1) {
    ...
    resultsStr += results.map(
      (result) => `<a href="//${Config.routes.dex}/moves/${toID(result)}" target="_blank" class="subtle" style="white-space:nowrap">${result}</a>${sort ? ` (${dex[toID(result)][sort.slice(0, -1)] === true ? "-" : dex[toID(result)][sort.slice(0, -1)]})` : ""}`
    ).join(", ");
    if (notShown) {
      resultsStr += `, and ${notShown} more. <span style="color:#999999;">Redo the search with ', all' at the end to show all results.</span>`;
    }
