...
class Auction extends Rooms.SimpleRoomGame {
  ...
  generateUsernameList(players, max = players.length, clickable = false) {
    ...
    buf += players.slice(0, max).map((p) => {
      if (typeof p === "object") {
        return `<username title="Tiers: ${p.tiers?.length ? `${import_lib.Utils.escapeHTML(p.tiers.join(", "))}` : "N/A"}"${clickable ? ' class="username"' : ""} style="font-weight: normal">${import_lib.Utils.escapeHTML(p.name)}</username>`;
      }
      return `<username${clickable ? ' class="username"' : ""} style="font-weight: normal">${import_lib.Utils.escapeHTML(p)}</username>`;
    }).join(", ");
  ...
  generatePriceList() {
    ...
    for (const team of this.teams.values()) {
      buf += import_lib.Utils.html`<details><summary>${team.name}</summary><table>`;
      for (const player of players.filter((p) => p.team === team)) {
        buf += import_lib.Utils.html`<tr><td>${player.name}</td><td>${player.price}</td></tr>`;
      }
  ...
  sendBidInfo() {
    ...
    buf += import_lib.Utils.html`Tiers: <b>${this.nominatedPlayer.tiers?.length ? `${this.nominatedPlayer.tiers.join(", ")}` : "N/A"}</b>`;
    buf += `</div>`;
  ...
  addTeam(name) {
    if (this.state !== "setup")
      throw new Chat.ErrorMessage(`You cannot add teams after the auction has started.`);
    if (name.length > 40)
  ...
  removeTeam(name) {
    if (this.state !== "setup")
      throw new Chat.ErrorMessage(`You cannot remove teams after the auction has started.`);
  ...
  suspendTeam(name) {
    ...
    if (this.nominatingTeam === team)
      throw new Chat.ErrorMessage(`You cannot suspend the current nominating team.`);
    team.suspended = true;
  }
  unsuspendTeam(name) {
    if (this.state !== "setup" && this.state !== "nom") {
  ...
  removeManagers(users) {
    for (const name of users) {
      if (!this.managers.delete(toID(name)))
        throw new Chat.ErrorMessage(`User "${name}" is not a manager.`);
    }
  }
  ...
  nominate(user, target) {
    ...
    this.sendMessage(import_lib.Utils.html`/html <username class="username">${user.name}</username> from team <b>${this.nominatingTeam.name}</b> has nominated <username>${player.name}</username> for auction. Use /bid or type a number to place a bid!`);
    if (!this.blindMode)
      this.sendBidInfo();
  ...
  bid(user, bid) {
    ...
    if (this.blindMode) {
      ...
      this.highestBid = bid;
      this.highestBidder = team;
      this.sendMessage(import_lib.Utils.html`/html <username class="username">${user.name}</username>[${team.name}]: <b>${bid}</b>`);
      this.sendBidInfo();
      this.clearTimer();
      this.bidTimer = setInterval(() => this.pokeBidTimer(), 1e3);
    }
  ...
  undoLastNom() {
    if (this.state !== "nom")
      throw new Chat.ErrorMessage(`You cannot undo a nomination right now.`);
    if (!this.lastQueue)
      throw new Chat.ErrorMessage(`You cannot undo more than one nomination at a time.`);
...
const commands = {
  auction: {
    ...
    bid(target, room, user) {
      const auction = this.requireGame(Auction);
      if (!target)
        return this.parse("/help auction bid");
