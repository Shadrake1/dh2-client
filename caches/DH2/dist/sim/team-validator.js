...
class TeamValidator {
  ...
  validateSet(set, teamHas) {
    ...
    if (!setSources.sourcesBefore && setSources.sources.length) {
      ...
      for (const source of setSources.sources) {
        if (["2E", "3E"].includes(source) && set.level < 5)
          continue;
  ...
  validateSource(set, source, setSources, species, because) {
    ...
    if (source.charAt(1) === "S") {
      ...
      if (source === "9D") {
        ...
        if (this.dex.currentMod === "moderngen3" || this.dex.currentMod === "moderngen4")
          return;
        eventData = {
          generation: 5,
          level: 10,
  ...
  checkCanLearn(move, originalSpecies, setSources = this.allSources(originalSpecies), set = {}) {
    ...
    for (const { species, learnset } of fullLearnset) {
      ...
      while (nextSpecies) {
        if (nextSpecies.name === species.name)
          break;
        nextSpecies = dex.species.learnsetParent(nextSpecies);
      }
      if (checkingPrevo && !nextSpecies)
        formeCantInherit = true;
      ...
      for (let learned of sources) {
        ...
        const canUseAbilityPatch = dex.gen >= 8 && format.mod !== "gen8dlc1";
        if (learnedGen < dex.gen && setSources.isHidden && !canUseAbilityPatch && !dex.mod("gen" + learnedGen).species.get(baseSpecies.name).abilities["H"]) {
